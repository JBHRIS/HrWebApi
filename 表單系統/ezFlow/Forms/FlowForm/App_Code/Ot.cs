...
public class Ot : System.Web.Services.WebService
{
    ...
    public bool Run(int ID)
    {
        ...
        foreach (var rAppS in dtAppS)
        {
            ...
            if (rAppS.bSign)
            {
                JBHR.Dll.Att.OtCal.OtSave(rAppS.sNobr, rAppS.sRoteCode, rAppS.dDateB, rAppS.sTimeB, rAppS.sTimeE, rAppS.sOtcatCode, rAppS.sOtrcdCode, rAppS.sOtDeptCode != null ? rAppS.sOtDeptCode : rAppS.sDept, 0, rAppS.sNote, rAppM.sName + "e", 0, "", rAppS.bExceptionHour);
                rAppS.sState = "3";

                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之加班單(已核准)";
            }
            else
            {
                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之加班單(已退簽)";
            }
