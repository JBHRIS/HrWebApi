...
SELECT      TOP (100) PERCENT MD.NOBR AS 員工編號, B.NAME_C AS 員工姓名, MD.ADATE AS 用餐日期, MT.MEALTYPE_CODE AS 用餐種類代碼, 
                   ...
                   CASE MD.OT WHEN 1 THEN '有' ELSE '無' END AS 加班, CASE MD.Eat WHEN 1 THEN '有' ELSE '無' END AS 用餐, 
                   MG.MealGroup_DISP AS 用餐群組代碼, MG.MealGroup_Name AS 用餐群組名稱, D.D_NO_DISP AS 部門代碼, D.D_NAME AS 部門名稱, 
                   MD.NOTE AS 備註, MD.SERO AS 序號, MD.KEY_MAN AS 登錄者, MD.KEY_DATE AS 登錄日期, MD.AutoKey
...
WHERE      (CONVERT(nvarchar(50), GETDATE(), 111) BETWEEN BT.ADATE AND BT.DDATE)
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 2, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'View_MealDeduction';
