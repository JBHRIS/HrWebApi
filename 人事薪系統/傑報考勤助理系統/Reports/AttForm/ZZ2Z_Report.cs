    ...
    public partial class ZZ2Z_Report : JBControls.JBForm
    {
        ...
        private void ZZ2Z_Report_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //出勤資料
                string sqlAttend = "select a.nobr,a.adate,b.rote_disp as rote,b.rotename,b.rote_sname,b.on_time,b.off_time,a.late_mins,a.e_mins,";
                ...
                if (labchedk)
                {
                    ...
                    string sqlOtd = "select a.nobr,a.bdate,a.btime,a.etime,f.on_time,f.off_time,e.rote from ot_b a";
                    ...
                    {
                        bool chantime = true;
                        if (!holicode.Contains(Row["rote"].ToString().Trim()))
                        {
                            decimal ontime = Convert.ToDecimal(Row["on_time"].ToString());
                            decimal offtime = Convert.ToDecimal(Row["off_time"].ToString());
                                ...
                                else
                                {
                                    if (row2 != null && row1 != null)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(row1["t2"].ToString()))
                                        {
                                            ...
                                            string tt2 = Row["off_time"].ToString();
                                            string t2 = Row["off_time"].ToString();
                                            ...
                                            decimal _attofftime = Convert.ToDecimal(_otbdate + Row["off_time"].ToString());
                                            ...
                                            int offtime = int.Parse(Row["off_time"].ToString());
                                            ...
                                            if (cardt1 < ontime && (ontime - cardt1 > 15) && offtime == otbtime)    //判斷上班卡時間小於上班時間,調整刷卡時間
                                            {
                                                if (t1.ToString().Length == 4)
                                                {
                                                    ...
                                                    if (absbtime != Row["on_time"]) row1["t1"] = _hstime;
                                                }
                                                if (tt1.ToString().Length == 4)
                                                {
                                                    ...
                                                    if (absbtime != Row["on_time"]) row1["tt1"] = _hstime;
                                                }
                                            }
                                            if (cardt2 > offtime && (cardt2 - offtime > 15) && ontime == otetime)   //判斷下班刷卡時間大於下班時間,調整刷卡時間
                                            {
                                                if (t2.ToString().Length == 4)
                                                {
                                                    ...
                                                    _hstime = Convert.ToString(decimal.Round(_etimeh, 0)).PadLeft(2, '0') + Convert.ToString(decimal.Round(_etimes, 0)).PadLeft(2, '0');
                                                    //row1["t2"] = (_etimes < 0) ? _hstime + "-" : _hstime;
                                                    if (absetime != Row["off_time"]) row1["t2"] = _hstime;
                                                }
                                                if (t2.ToString().Length == 4)
                                                {
                                                    ...
                                                    _hstime = Convert.ToString(decimal.Round(_etimeh, 0)).PadLeft(2, '0') + Convert.ToString(decimal.Round(_etimes, 0)).PadLeft(2, '0');
                                                    //row1["tt2"] = (_etimes < 0) ? _hstime + "-" : _hstime;
                                                    if (absetime != Row["off_time"]) row1["tt2"] = _hstime;
                                                }
                                            }
                                        }
                                    }
                    ...
                    foreach (DataRow Row in SRowAtt)
                    {
                        ...
                        if (row1 != null)
                        {
                            ...
                            else if (!string.IsNullOrEmpty(row1["t2"].ToString()))
                            {
                                int offtime = int.Parse(Row["off_time"].ToString());
                                int ontime = int.Parse(Row["on_time"].ToString());
                                int cardt1 = int.Parse(row1["t1"].ToString());
                ...
                rq_attendn = null; rq_countatt = null; rq_countatt1 = null; rq_salatt = null; rq_salatt1 = null;
                ...
                {                   
                    ...
                    {
                        RptViewer.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("NIGAMT", NIGAMT) });
                        RptViewer.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("FOODAMT", FOODAMT) });
                        RptViewer.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("SPECAMT", SPECAMT) });
