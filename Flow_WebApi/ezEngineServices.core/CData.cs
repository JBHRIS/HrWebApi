...
namespace ezEngineServices.core
{
    public class CData
    {
        ...
        public CData(ezFlowContext dcFlow)
        {
            this.dcFlow = dcFlow;
        }
        ...
        public List<ProcessDataRow> GetProcessData(List<int> lsProcessID)
        {
            ...
            var rsFlowNode = (from c in dcFlow.FlowNodes
                              select c).ToList();
            ...
            var rsProcessMultiFlow = (from pn in dcFlow.ProcessMultiFlows
                                      where (from pf in rsProcessFlowSql where pn.ProcessFlow_id == pf.id select 1).Any()
                                      select pn).ToList();
            ...
            foreach (var rProcessFlow in rsProcessFlow)
            {
                ...
                var rEmp = (from c in dcFlow.Emps
        ...
        {
            var rsFlowTree = (from t in dcFlow.FlowTrees
            ...
            var rsFlowTreePower = (from t in dcFlow.FlowTreePowers
                                   select t).ToList();

            var rsFlowTreePowerRoleOnly = (from t in dcFlow.FlowTreePowerRoleOnlies
                                           select t).ToList();

            var rsFlowNode = (from n in dcFlow.FlowNodes
                              join ns in dcFlow.NodeStarts on n.id equals ns.FlowNode_id into nns
                              from nnsRow in nns.DefaultIfEmpty()
                              join nf in dcFlow.NodeForms on n.id equals nf.FlowNode_id into nnf
        ...
        {
            var ProcessSql = from pc in dcFlow.ProcessChecks
                             join pn in dcFlow.ProcessNodes on pc.ProcessNode_auto equals pn.auto
                             ...
                             join e1 in dcFlow.Emps on pc.Emp_idDefault equals e1.id into pc1
                             from pc1Row in pc1.DefaultIfEmpty()
                             join e2 in dcFlow.Emps on pc.Emp_idAgent equals e2.id into pc2
            ...
            //填入表單資訊
            var rsFormApp = (from c in dcFlow.wfFormApps
            ...
            var rsProcessApParm = (from c in dcFlow.ProcessApParms
            ...
            , int iProcessID = 0, string sApp = "1")
        {
            ...
            var arrForm = (from c in dcFlow.wfForms
                           where c.iSort > 0
                           orderby c.iSort
            ...
            {
                //審核者
                var rsProcessNode = (from pn in dcFlow.ProcessNodes
                                     join pc in dcFlow.ProcessChecks on pn.auto equals pc.ProcessNode_auto
                ...
                //被申請人
                var rsFormAppInfo = (from s in dcFlow.wfFormAppInfos
                ...
                //申請人
                Vdb = (from m in dcFlow.wfFormApps
                       join pf in dcFlow.ProcessFlows on m.idProcess equals pf.id
                       join f in dcFlow.wfForms on pf.FlowTree_id equals f.sFlowTree
        ...
        public List<CheckAgentDataRow> GetCheckAgentData(string EmpId)
        {
            ...
            var rsCheckAgent = (from ca in dcFlow.CheckAgents
                                where ca.Emp_idSource == EmpId
                                select ca).ToList();

            var lsGuid = rsCheckAgent.Select(p => p.Guid).ToList();

            var rsForm = (from c in dcFlow.CheckAgentFlowTrees
                          join d in dcFlow.wfForms on c.FlowTree_id equals d.sFlowTree
            ...
            var rsRole = (from c in dcFlow.Roles
            ...
            var rsDept = (from c in dcFlow.Depts
        ...
        {
            var Vdb = (from c in dcFlow.CheckAgentFlowTrees
                       join f in dcFlow.FlowTrees on c.FlowTree_id equals f.id
        ...
        {
            var Vdb = (from c in dcFlow.EmpAgentDates
        ...
        {
            var ProcessSql = from pf in dcFlow.ProcessFlows
                             join pn in dcFlow.ProcessNodes on pf.id equals pn.ProcessFlow_id
                             join pc in dcFlow.ProcessChecks on pn.auto equals pc.ProcessNode_auto
                             join pa in dcFlow.ProcessApViews on pf.id equals pa.ProcessFlow_id
                             join ft in dcFlow.FlowTrees on pf.FlowTree_id equals ft.id
                             join fn in dcFlow.FlowNodes on pn.FlowNode_id equals fn.id
                             join pfs in dcFlow.ProcessFlowShares on pf.id equals pfs.ProcessFlow_id
                             join e1 in dcFlow.Emps on pc.Emp_idAgent equals e1.id into pc1
                             from pc1Row in pc1.DefaultIfEmpty()
                             join e2 in dcFlow.Emps on pc.Emp_idDefault equals e2.id into pc2
                             from pc2Row in pc2.DefaultIfEmpty()
                             where !pf.isFinish.Value
                             && !pf.isCancel.Value
                             && !pf.isError.Value
        ...
        {
            var Vdb = (from pf in dcFlow.ProcessFlows
                       join pav in dcFlow.ProcessApViews on pf.id equals pav.ProcessFlow_id.Value
                       join ft in dcFlow.FlowTrees on pf.FlowTree_id equals ft.id
                       where pf.isFinish.Value
                       && !pf.isCancel.Value
                       && !pf.isError.Value
        ...
        {
            var Vdb = (from fn in dcFlow.FlowNodes
                       join ns in dcFlow.NodeStarts on fn.id equals ns.FlowNode_id
                       where fn.nodeType == "1"
                       && (lsFlowTreeId == null
        ...
        public List<FlowNodeApNameRow> GetFlowNodeAppName(List<string> lsFlowNodeId)
        {
            ...
            foreach (var rFlowNode in rsFlowNode)
            {
                ...
                {
                    case "3": //主管審核
                        var rNodeMang = (from c in dcFlow.NodeMangs
                        ...
                        break;
                    case "4": //流程起始者
                        var rNodeInit = (from c in dcFlow.NodeInits
                                         where c.FlowNode_id == rFlowNode.id
                                         select c).FirstOrDefault();
                        ...
                        break;
                    case "5": //會簽起始者
                        var rNodeMultiInit = (from c in dcFlow.NodeMultiInits
                                              where c.FlowNode_id == rFlowNode.id
                                              select c).FirstOrDefault();
                        ...
                        break;
                    case "6": //自定簽核者
                        var rNodeCustom = (from c in dcFlow.NodeCustoms
                                           where c.FlowNode_id == rFlowNode.id
                                           select c).FirstOrDefault();
                        ...
                        break;
                    case "7": //動態簽核者
                        var rNodeDynamic = (from c in dcFlow.NodeDynamics
                                            where c.FlowNode_id == rFlowNode.id
                                            select c).FirstOrDefault();
                        ...
                        break;
                    case "8": //流程代理者
                        var rNodeAgentInit = (from c in dcFlow.NodeAgentInits
                                              where c.FlowNode_id == rFlowNode.id
                                              select c).FirstOrDefault();
        ...
        {
            string Url = "";
            var rNodeStart = (from pf in dcFlow.ProcessFlows
                              join fn in dcFlow.FlowNodes on pf.FlowTree_id equals fn.FlowTree_id
                              join ns in dcFlow.NodeStarts on fn.id equals ns.FlowNode_id
            ...
            if (rNodeStart != null)
            {
                var rProcessApView = (from c in dcFlow.ProcessApViews
        ...
        public string GetFlowParmUrl(int iApParmID, bool bOnlyUrl = false)
        {
            ...
            if (iApParmID != 0)
            {
                var rSysVar = (from c in dcFlow.SysVars
                               select c).FirstOrDefault();
                ...
                if (rSysVar != null)
                {
                    ...
                    if (rProcessApParm != null)
                    {
                        ...
                        if (rProcessFlow != null)
                        {
                            ...
                            if (rFlowNode != null)
                            {
                                var rNodeStart = (from c in dcFlow.NodeStarts
                                                  where c.FlowNode_id == rFlowNode.id
                                                  select c).FirstOrDefault();
                                ...
                                if (rNodeStart != null)
                                {
                                    ...
                                    {
                                        var rFlowNodeCheck = (from c in dcFlow.FlowNodes
