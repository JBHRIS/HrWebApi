...
public partial class Ot_Std : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        //oTransCardDao.TransCard(sNobr, sNobr, "0", "z", dDateB, dDateB, OtRote, sNobr, true, true, true, "", "JB-TRANSCARD", true, 1);
        ...
        //生技特別需求 20130531 之前只能申請補休 20130601 之後只能申請加班 20130531 by 瑜瑄
        //DateTime dDate20130531 = new DateTime(2013, 5, 31);
        //if ((",AA04,").IndexOf("," + rDataGroup.sComp + ",") >= 0)
        //{
        //    //if (dDateB.Date > dDate20130531.Date && sOtCat == "2")
        //    //{
        //    //    lblMsg.Text = "您不可以選擇換補休假";
        //    //    ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
        //    //    return;
        //    //}

        //    if (dDateB.Date <= dDate20130531.Date && sOtCat == "1")
        //    {
        //        lblMsg.Text = "您不可以選擇換加班費";
        //        ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
        //        return;
        //    }
        //}
        //else if ((",AA01LT,AA01LT,AA01LT,AA01LZ,AA01LZ,").IndexOf("," + rDataGroup.sComp + ",") >= 0)
        //{
        //    //龍潭 某些人不可以申請加班費 所有間接人員都不可以申請 僅有組長假日例外
        //    if (rEmpBaseS.sDI == "I" && sOtCat == "1")
        //    {
        //        if (rJob.name.Trim() == "組長" && rRote.sRoteCode == "00")
        //        {
        //        }
        //        else
        //        {
        ...
        //if (ddlRote.ToolTip != "00")
        //{
        //    if (bEat)
        //    {
        //        lblMsg.Text = "非假日不允許勾沒用餐或沒休息";
        //        ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
        //        return;
        //    }
        ...
        //    if (rEmpBaseS.sDI == "I")
        //    {
        //        lblMsg.Text = "間接人員平日不可申請";
        //        ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
        //        return;
        //    }

        //    if (ddlRote.ToolTip != OtRote)
        //    {
        //        lblMsg.Text = "非假日不允許更換加班班別";
        //        ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
        //        return;
        //    }
        //}
        ...
        if (!JBHR.Dll.Att.OtCheck.IsCardTime(sNobr, dDateB, sTimeB, sTimeE))// && rR.sRoteCode != "00")
        {
            lblMsg.Text = "您的時間不在刷卡時間裡";
            ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
            return;
        }
        ...
        //檢查重複的資料
        var rsAppS = from c in dtAppS
                     where c.dDateTimeB < dDateTimeE && c.dDateTimeE > dDateTimeB
                     select c;
        ...
        //var rAttEnd = JBHR.Dll.Att.Attend(sNobr, dDateB);

        //if (oOtDetail.bDifferenceRote)
        //{
        //    lblMsg.Text = "加班班別與平時上班班別不符合(警告訊息)";
        //    ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
        //}

        //if (rR.sRoteCode == "00" && oOtDetail.bTimeError)
        //{
        //    lblMsg.Text = "假日加班時間與班別時間完全沒有交集";
        //    ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
        //    return;
        //}
        ...
        //天然災害日不需要卡時數
        if (oOtDetail.iHour < iOtUnit && !rOtrcd.bNoCalc)
        {
            lblMsg.Text = "加班時數未大於" + iOtUnit.ToString() + "小時";
            ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
            return;
        }

        if (oOtDetail.iHour < 1)
        {
            lblMsg.Text = "加班時數必須大於1小時";
            ScriptManager.RegisterStartupScript(updatePanel1, typeof(UpdatePanel), "key", "alert('" + lblMsg.Text + "');", true);
            return;
        }

        string sYYMM = JBHR.Dll.Att.SetYYMM(dDateB, "2", 1, rEmpBaseS.sSaladr);
        ...
        rs.sInfo = rs.sName + "," + rs.sOtcatName + "," + rs.dDateB.ToShortDateString() + "," + rs.sTimeB + "," + rs.dDateE.ToShortDateString() + "," + rs.iTotalHour + "小時," + rs.sNote;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        var lsAppS = (from c in dcForm.wfAppOt
    ...
    protected void txtDateB_TextChanged(object sender, EventArgs e)
    {
        ...
        var rEmp = JBHR.Dll.Bas.EmpBase(lblNobr.Text).FirstOrDefault();
        if (rEmp != null && rEmp.sCarcd == "02")
        {
            txtTimeB.Text = "0000";
        }
    ...
    {
        //var rOt1 = JBHR.Dll.Att.Ot1(lblNobr.Text, dDate, dDate).FirstOrDefault();
        //if (rOt1 != null)
        //    lblOt1.Text = rOt1.sTimeB + "-" + rOt1.sTimeE + "(" + (rOt1.iOtHrs + rOt1.iRestHrs).ToString() + ")";
    ...
    protected void btnConvert_Click(object sender, EventArgs e)
    {
        //再一次刷卡轉出勤
        string sNobr = lblNobr.Text.Trim();
        string sRote = ddlRote.SelectedItem.Value;
        DateTime dDateB = DateTime.Now;
        if (!DateTime.TryParse(txtDateB.Text, out dDateB))
        {
            return;
        ...
        try
        {
            Dal.Dao.Att.TransCardDao target = new Dal.Dao.Att.TransCardDao(dcHR.Connection);
            target.TransCard(sNobr, sNobr, "0", "z", dDateB, dDateB, sRote, sNobr, true, true, true, "", "JB-TRANSCARD", true, 1);
        }
        catch (Exception ex) { }

        SetTime(sNobr, dDateB);
    }
}