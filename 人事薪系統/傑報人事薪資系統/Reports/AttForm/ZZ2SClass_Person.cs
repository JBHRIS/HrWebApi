...
namespace JBHR.Reports.AttForm
{
    class ZZ2SClass_Person
    {
        public static void DoSend(string nobr_b, string nobr_e, string date_b, string date_e, string TestSubject, string MailFrom, string HRMail, string HRMail1, string HRMail2, bool TestSend, string type_data, string Dept)
        {
            ...
            CmdAttend += ",c.d_name,d.email,c.email as deptmail";
            ...
            CmdAttend += string.Format(@" and a.nobr between '{0}' and '{1}'", nobr_b, nobr_e);
            CmdAttend += " and b.card='Y' and (a.late_mins!=0 or a.e_mins!=0 or abs=1 )";//or forget > 0
            CmdAttend += " and d.email<>'' ";
            CmdAttend += " order by b.deptm,a.nobr,a.adate";
            ...
            foreach (DataRow Row0 in rq_email.Rows)
            {
                ...
                if (Dt.Rows.Count > 0)
                {
                    ...
                    if (TestSend)
                    {
                        if (HRMail != "") Smail.SendMailWithQueue(MailFrom, HRMail, date_b + "  - " + date_e + _Subject + TestSubject, html);
                        if (HRMail1 != "") Smail.SendMailWithQueue(MailFrom, HRMail1, date_b + "  - " + date_e + _Subject + TestSubject, html);
                        if (HRMail2 != "") Smail.SendMailWithQueue(MailFrom, HRMail2, date_b + "  - " + date_e + _Subject + TestSubject, html);
                    }
                    else
                    {                       
