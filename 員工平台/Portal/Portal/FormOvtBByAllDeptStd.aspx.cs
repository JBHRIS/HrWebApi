...
namespace Portal
{
    public partial class FormOvtBByAllDeptStd : WebPageBase
    {
        //此為可申請全編制部門版本
        ...
        private void ckblNameAppS_DataBind()
        {
            if (lblDeptCodeAppM.Text.Trim().Length > 0)
            {
                ...
                lblDeptCodeAppMByDept.Text = ddlDept.SelectedValue;
        ...
        private void SetTime(string sNobr, DateTime dDate)
        {
            ...
            //if (txtRote.SelectedValue != null)
            //{
            //    OldDal.Dao.Att.RoteDao oRoteDao = new OldDal.Dao.Att.RoteDao(dcHR.Connection);
            //    var rRote = oRoteDao.GetRoteDetail(new List<string>() { txtRote.SelectedValue }).FirstOrDefault();
            ...
            //            if (rRote.OtBeginTime.CompareTo("2400") >= 0)
            //            {
            //                txtTimeE.Text = (Convert.ToInt32(rRote.OtBeginTime) - 2400).ToString("0000");
            //                txtTimeB.Text = (Convert.ToInt32(rRote.OtBeginTime) - 2400).ToString("0000");
            //            }
            //            else
            //                txtTimeE.Text = rRote.OtBeginTime;
            //        }
        ...
        protected void txtDateB_SelectedDateChanged(object sender, Telerik.Web.UI.Calendar.SelectedDateChangedEventArgs e)
        {
            ...
            SetTime(null, txtDateB.SelectedDate.Value);
            //SetRote(Nobr, Date);
            ...
            foreach (ListItem item in ckblNameAppS.Items)
            {
                if (item.Selected == true)
                {
                    ...
                    if (GetAttend != null)
                    {
                        ...
                        {
                            lblErrorMsg.Text = lblNobrAppS.Text + "無此出勤班別，請假人事單位";
                            return;
                        }
                    }
                    else
                    {
                        lblErrorMsg.Text = lblNobrAppS.Text + "出勤班別錯誤，請假人事單位";
                        return;
                    }
                    ...
                    if (DateTimeB < DateTime.Now)
                    {
                        ...
                        {
                            lblErrorMsg.Text = lblNobrAppS.Text + "所申請的時間不在刷卡或公出時間裡";
                            return;
                        }
                    }
                    ...
                    {
                        lblErrorMsg.Text = lblNobrAppS.Text + "資料重複或流程正在進行中";
                        return;
                    }
                    ...
                    foreach (var c in rsOt)
                    {
                        ...
                        {
                            lblErrorMsg.Text = lblNobrAppS.Text + "人事資料重複";
                            return;
                        }
                    }
                    ...
                    if (jg == 0)
                    {
                        ...
                        if (OtIntervalExtend.Any())
                        {
                            ...
                            Calculate = oOtDao.GetCalculate(Nobr, OtCatCode, DateB1, DateE1, TimeB1, TimeE1, Otrcd, 0, RoteHCode, bTime24: true, bCalculateRes: !noCalc, InterVal: IntervalTime);
                        }
                        else
                            Calculate = oOtDao.GetCalculate(Nobr, OtCatCode, DateB1, DateE1, TimeB1, TimeE1, Otrcd, 0, RoteHCode, bTime24: true, bCalculateRes: !noCalc);

                        jg = 1;
                    }
                    if (Calculate == 0M)
                    {
                        lblErrorMsg.Text = lblNobrAppS.Text + "計算時數必須大於0.5小時";
                        return;
                    }
                    ...
                    AttDateCycleCond.date = IsNightShift ? DateB.AddDays(-1) : DateB;
