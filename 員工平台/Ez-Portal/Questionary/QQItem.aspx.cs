...
{
    private const string qqItemEditUrl = @"~/Questionary/QQItemEdit.aspx?Id=";
    ...
    protected void gv_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        ...
        gv.DataSource = (from c in qqItemRepo.GetAll_Dlo().OrderByDescending(p => p.CreatedDate)
                         select new
                         {
                             c.Id,
                             c.TypeCode,
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        if (cbType.SelectedValue.Equals(QQTypeEnum.MCQ.ToString()))
        {
            ...
            qqItemObj.IsRequired = cbxIsRequired.Checked;
            if (cbxIsRequired.Checked)
            {
                qqItemObj.MinCharCount = rtbxMinCharCount.Text.Length > 0 ? Convert.ToInt32(rtbxMinCharCount.Text) : 0;
            }
    ...
    protected void cbType_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
    {
        ...
        if (e.Value == QQTypeEnum.SAQ.ToString())
        {
            Label1.Visible = true;
            rtbxMinCharCount.Visible = true;
        }
        else
        {
            Label1.Visible = false;
            rtbxMinCharCount.Visible = false;
        }
    }
