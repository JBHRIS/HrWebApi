...
namespace ezEngineServices.core
{
    public class CFlow
    {
        ...
        public CFlow(ezFlowContext dcFlow)
        {
            this.dcFlow = dcFlow;
        }
        ...
        public bool IsNextManage(int idProcess, int idProcessNode_Source, int idProcessCheck_Source, string idFlowNode)
        {
            ...
            var r1 = (from c in dcFlow.ProcessNodes
                      join n in dcFlow.FlowNodes on c.FlowNode_id equals n.id
            ...
            if (r1 != null && r1.nodeType != "3")
            {
                ...
                {
                    var r2 = (from c in dcFlow.ProcessNodes
                              join n in dcFlow.FlowNodes on c.FlowNode_id equals n.id
            ...
            var rsNodeMangLoopBreak = (from c in dcFlow.NodeMangLoopBreaks
            ...
            if (rsNodeMangLoopBreak.Count > 0)
            {
                ...
                foreach (var rNodeMangLoopBreak in rsNodeMangLoopBreak)
                {
                    ...
                    try
                    {
                        results = dcFlow.wfFormApps.FromSqlRaw(SqlCommandText).Count();
                    }
                    catch (Exception ex)
                    {
        ...
        public List<string> GetLinkNextNode(int idProcess, int idProcessNode_Source, int idProcessCheck_Source, string idFlowTree, string idFlowNode_Source)
        {
            ...
            var rsFlowLinkSql = from c in dcFlow.FlowLinks
                                where c.FlowTree_id == idFlowTree
                                && c.FlowNode_idSource == idFlowNode_Source
                                select c;

            var rsFlowLinkPower = (from c in dcFlow.FlowLinkPowers
                    ...
                    switch (rFlowNode.nodeType)
                    {
                        ...
                        case "4": //流程起始者
                            ...
                            else
                            {
                                ...
                                {
                                    using (var transaction1 = dcFlow.Database.BeginTransaction())
                                    {
                                        try
                                        {
                                            ...
                                            var rNodeDynamic = dcFlow.NodeInits.FirstOrDefault(p => p.FlowNode_id == sNode_Next);
                                ...
                                else
                                {
                                    using (var transaction1 = dcFlow.Database.BeginTransaction())
                                    {
                                        try
                                        {
                                            ...
                                            //&& p.isNextNodeAutoSign.Value 缺少欄位
                                            var rNodeDynamic = dcFlow.NodeDynamics.FirstOrDefault(p => p.FlowNode_id == sNode_Next);
                        ...
                        case "9": //會簽流程
                            {
                                ...
                                var rsProcessMultiFlowSql = from c in dcFlow.ProcessMultiFlows
                                                            where c.ProcessNode_auto == 0
                                                            && c.ProcessFlow_id == idProcess
                                                            select c;

                                var rsFlowNodeSql = from c in dcFlow.FlowNodes
                                ...
                                foreach (var rProcessMultiFlow in rsProcessMultiFlow)
                                {
                                    ...
                                    if (hasStart)
                                    {
                                        ...
                                        if (rFlowLink != null)
                                        {
                                            ...
                                            rFlowNode = (from c in dcFlow.FlowNodes
                        ...
                        case "11": //服務程式，直接過去吧！
                            ...
                            var rNodeService = (from c in dcFlow.NodeServices
                        ...
                        case "12": //流程結束
                            {
                                ...
                                if (rProcessMultiFlow != null)
                                {
                                    ...
                                    var rsProcessFlow = (from c in dcFlow.ProcessFlows
                                    ...
                                    if (isAllFinish)
                                    {
                                        rProcessFlow = (from c in dcFlow.ProcessFlows
