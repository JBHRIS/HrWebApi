...
public class Card : System.Web.Services.WebService
{
    ...
    public bool Run(int ID)
    {
        ...
        foreach (var rAppS in dtAppS)
        {
            ...
            if (rAppS.bSign)
            {
                JBHR.Dll.Att.CardCal.CardSave(rAppS.sNobr, rAppS.dDate, rAppS.sTime, rAppS.sReasonCode, rAppM.sName + "e", rAppS.sNote, rAppM.sProcessID);
                rAppS.sState = "3";

                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之忘刷單(已核准)";
            }
            else
            {
                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之忘刷單(已退簽)";
            }
