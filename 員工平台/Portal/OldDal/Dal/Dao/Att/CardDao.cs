...
namespace OldDal.Dao.Att
{
    public class CardDao
    {
        ...
        public bool Save(string sNobr, DateTime dDate, string sTime, string sReason, string sNote = "", string sKeyMan = "System", string sSerno = "", bool isLos= true,string IP="")
        {
            bool Vdb = false;

            var rCARD = (from c in dcHr.CARD
                         where c.NOBR.Trim() == sNobr
                         && c.ADATE.Date == dDate.Date
                         && c.ONTIME.Trim() == sTime
                         select c).FirstOrDefault();
            ...
            {
                rCARD = new CARD();
                OldBll.Tools.DefaultData.SetRowDefaultValue(rCARD);
                rCARD.NOBR = sNobr;
                rCARD.ADATE = dDate;
                rCARD.ONTIME = sTime;
                rCARD.CODE = "1";
        ...
        /// <param name="sSerno">序號</param>
        /// <returns>bool</returns>
        public bool SaveCard(string sNobr, DateTime dDate, string sTime, string sReason, string sNote = "", string sKeyMan = "System", string sSerno = "", bool isLos = true, string IP = "")
        {
            bool Vdb = false;
