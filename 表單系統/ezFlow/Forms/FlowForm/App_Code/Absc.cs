...
public class Absc : System.Web.Services.WebService
{
    public Absc() { }
    ...
    public bool Run(int ID)
    {
        ...
        foreach (var rAppS in dtAppS)
        {
            ...
            if (rAppS.bSign)
            {
                //JBHR.Dll.Att.Absc.AbsDelete(rAppS.sNobr, rAppS.dDate, rAppS.sTime, rAppS.sHcode, rAppM.sName + "e");
                oAbsDao.AbsDelete(rAppS.sNobr, rAppS.dDate, rAppS.sTime, rAppS.sHcode, rAppM.sName, rAppM.sProcessID);
                rAppS.sState = "3";

                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之銷假單(已核准)";
            }
            else
            {
                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之銷假單(已退簽)";
            }
