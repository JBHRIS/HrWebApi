...
public partial class Improve_CheckHR : System.Web.UI.Page
{
    ...
    {
        var rs = (from c in dcForm.wfAppImprove
                  where c.sProcessID == lblProcessID.Text
                  select c).FirstOrDefault();
        if (rs != null)
        {
            lblTitle1.Text = rs.sTitle;
            lblCat.Text = rs.sCatName;
            lblDate.Text = rs.dDate.Value.ToShortDateString();
            lblName.Text = rs.sName;
            cbName.Checked = rs.bName;
            lblNote.Text = rs.sNote.Replace("\n", "<BR>");
            lblUploadKey.Text = rs.sReserve4;
            lblDragNameUpload.Text = rs.sReserve4;
            lblMsgUpload.Text = "";
        }
    }
