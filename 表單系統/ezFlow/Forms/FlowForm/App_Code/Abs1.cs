...
public class Abs1 : System.Web.Services.WebService
{
    ...
    public bool Run(int ID)
    {
        ...
        foreach (var rAppS in dtAppS)
        {
            ...
            if (rAppS.bSign)
            {
                string sTimeE = rAppS.sTimeE == "0000" ? "2359" : rAppS.sTimeE;
                JBHR.Dll.Att.AbsCal.AbsSave(rAppS.sNobr, rAppS.sHcode, rAppS.dDateB, rAppS.dDateE, rAppS.sTimeB, sTimeE, "", rAppS.sNote, rAppM.sName + "e", rAppM.sReserve1 != null ? rAppM.sReserve1 : "", 0, Guid.NewGuid().ToString());
                rAppS.sState = "3";

                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之公出單(已核准)";
            }
            else
            {
                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之公出單(已退簽)";
            }
            ...
            var rEmpM = (from c in dcFlow.Emp
                         where c.id == rAppM.sNobr
                         select c).FirstOrDefault();

            if (rEmpM != null && rEmpM.email.Trim().Length > 0 && sBody != null && sBody.Length > 0)
            {
                ...
                rSendMail.sToAddress = rEmpM.email;
