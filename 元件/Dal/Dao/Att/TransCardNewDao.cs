...
namespace Dal.Dao.Att
{
    ...
    /// TransCardDao
    /// </summary>
    public class TransCardNewDao : JBModule.Message.ReportStatus
    {
        ...
        /// </summary>
        /// <param name="conn">連接字串 沒有等於預設</param>
        public TransCardNewDao(IDbConnection conn = null)
        {
        ...
        public TransCardNewDao(string ConnectionString = null)
        {
        ...
        private List<BaseTable> GetBaseByNobr(TransCardCondition Cond)
        {
            ...
            var Vdb = (from c in dcHr.BASETTS
                       where Cond.lsNobr.Contains(c.NOBR)
        ...
        private List<RoteTable> GetRote(string[] arrRoteCode = null)
        {
            ...
            if (arrRoteCode != null)
                iqRote = from c in dcHr.ROTE
                         where arrRoteCode.Contains(c.ROTE1)
                         select c;
            ...
            var Vdb = (from c in iqRote
                       select new RoteTable()
                       {
                           RoteCode = c.ROTE1,
                           OnTime = c.ON_TIME,
                           OffTime = c.ATT_END.Length > 0 ? c.ATT_END : c.OFF_TIME,
        ...
        private List<RoteResTable> GetRoteRes(string sResB1, string sResE1, string sResB2, string sResE2, string sResB3, string sResE3, string sResB4, string sResE4, string sResB5, string sResE5)
        {
            ...
            if (sResB1.Length == 4 && sResE1.Length == 4)
            {
                r = new RoteResTable();
                r.ResB = sResB1;
                r.ResE = sResE1;
            ...
            if (sResB2.Length == 4 && sResE2.Length == 4)
            {
                r = new RoteResTable();
                r.ResB = sResB2;
                r.ResE = sResE2;
            ...
            if (sResB3.Length == 4 && sResE3.Length == 4)
            {
                r = new RoteResTable();
                r.ResB = sResB3;
                r.ResE = sResE3;
            ...
            if (sResB4.Length == 4 && sResE4.Length == 4)
            {
                r = new RoteResTable();
                r.ResB = sResB4;
                r.ResE = sResE4;
            ...
            if (sResB5.Length == 4 && sResE5.Length == 4)
            {
                r = new RoteResTable();
                r.ResB = sResB5;
                r.ResE = sResE5;
        ...
        private List<AbsTable> GetAbs(TransCardCondition Cond)
        {
            var Vdb1 = (from a in dcHr.ABS
                        ...
                        where Cond.lsNobr.Contains(a.NOBR)
        ...
        public int TransCardPool(TransCardCondition oCond)
        {
            ...
            int _ThreadAvg = oCond.lsNobr.Count / _ThreadCount;
            TransCardPools tcp;
            ...
            for (int count = 0; count < oCond.lsNobr.Count; count++)
            {
                this.Report(50, "判斷異常中..." + oCond.lsNobr[count]);

                tcp = new TransCardPools(dcHr.Connection, null, Vdb, oCond, new List<string>(), oCond.lsNobr[count], iPass);
                        ...
                        if (rBaseData != null)
                        {
                            if (oCond.sRote == "0" || oCond.sRote.Trim().Length == 0)
                            {
                                ...
                                if (rRote != null && IsHoliDay(Vdb.RoteData, RoteCode))
                                {
                                    ...
                                    if (rOt != null)//如果加班班別也是假日班的時候，參考出勤資料的加班班別(ROTE_H)
