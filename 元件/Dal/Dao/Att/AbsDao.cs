...
namespace Dal.Dao.Att
{
    public class AbsDao
    {
        ...
        public List<AbsDataTable> GetAbs(string sNobr, DateTime dDateB, DateTime dDateE)
        {
            ...
            var Vdb = Vdb1.Union(Vdb2)
                .Select(a => new AbsDataTable()
                {
                    ...
                    NameA = a.NameA,
                    NoCal = a.NoCal,
                    DateTimeB = a.DateB.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(a.TimeB)),
                    DateTimeE = a.DateE.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(a.TimeE))
                }).ToList();
        ...
        public List<AbsDataTable> GetAbs(string sNobr, DateTime dDateB, DateTime dDateE, bool bAdd = false)
        {
            var Vdb1 = (from a in dcHr.ABS
                        ...
                        select new AbsDataTable()
                        {
                            ...
                            HcodeUnit = b.UNIT.Trim() == "天" ? Bll.MT.mtEnum.HcodeUnit.Day : Bll.MT.mtEnum.HcodeUnit.Hour,
                            Use = a.TOL_HOURS,
                            NameA = a.A_NAME != null ? a.A_NAME.Trim() : "",
                            NoCal = a.nocalc.GetValueOrDefault(false),
                            Serno = a.SERNO.Trim(),
        ...
        public int DeleteAbs(string sNobr, DateTime dDateB, string sTimeB, DateTime dDateE, string sTimeE, string sHcode)
        {
            ...
            foreach (var rABS in rsABS)
            {
                dDateTimeB = rABS.BDATE.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(rABS.BTIME.Trim()));
                dDateTimeE = rABS.BDATE.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(rABS.ETIME.Trim()));
        ...
        public AbsCalculateRow GetCalculate(string sNobr, string sHcode, DateTime dDateB, DateTime dDateE, string sTimeB, string sTimeE, bool bCalculateWorkTime = true, bool bCalculateRes = true, decimal iException = 0, bool bFixedCycle = false, string sRoteCode = "", bool bTime24 = false)
        {
            ...
            //套用請假計算公式
            Bll.Att.Abs oAbs = new Bll.Att.Abs();
            ...
            for (DateTime dDay = dDateB.Date; dDay <= dDateE.Date; dDay = dDay.AddDays(1).Date)
            {
                ...
                if (!(rAttend != null && rAttend.IsHoliDay && !rHcode.InHoli) || (dDateB.Date == dDateE.Date && rAttend.RoteCode == "0X"))
                {
                    ...
                    if (!lsRoteHoliDayRoteCode.Contains(CalculateRoteCode))
                    {
                        ...
                        if (rRote != null)
                        {
                            DateTime dRoteDateTimeB = dDay.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(rRote.OnTime));
                            DateTime dRoteDateTimeE = dDay.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(rRote.AbsEndTime));
                            ...
                            DateTime dDateTimeB = dDay.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeCalB));
                            DateTime dDateTimeE = dDay.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeCalE));
                            ...
                            //特休以上半天及下半天來寫入
                            if (sHcode == "A" && rRote.MiddleTimeB.Trim().Length > 0 && rRote.MiddleTimeE.Trim().Length > 0)
                            {
                                if (rHcode.Unit == Bll.MT.mtEnum.HcodeUnit.Day)
                                {
                                    if (sTimeCalB.CompareTo(rRote.MiddleTimeB) < 0)
                                        iUse += 0.5M;

                                    if (sTimeCalE.CompareTo(rRote.MiddleTimeE) > 0)
                                        iUse += 0.5M;
                                }
                                else
                                {
                            ...
                            rDay.DateTimeB = dDay.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeCalB));
                            rDay.DateTimeE = dDay.Date.AddMinutes(Bll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeCalE));
            ...
            foreach (var rHcode0246 in rsHcode0246)
            {
                ...
                if (rHcode0246.YearRest == "0")
                {
                    ...
                    else
                    {
                        ...
                        {
                            DateTime dDateCalB = rHcode0246.CalUnit == Bll.MT.mtEnum.HcodeUnit.Year ? dDateYearB : dDateMonthB;
                            DateTime dDateCalE = rHcode0246.CalUnit == Bll.MT.mtEnum.HcodeUnit.Year ? dDateYearE : dDateMonthE;
                ...
                rAbsBalanceRow.Balance = (rHcode0246.Sex == Bll.MT.mtEnum.SexCategroy.Both || rHcode0246.Sex == rBase.Sex) ? iBalance : 0;
                rAbsBalanceRow.RealBalance = (rHcode0246.Sex == Bll.MT.mtEnum.SexCategroy.Both || rHcode0246.Sex == rBase.Sex) ? iRealBalance : 0;
                rAbsBalanceRow.MaxGroup = iMaxGroup;
                rAbsBalanceRow.UseGroup = iUseGroup;
                rAbsBalanceRow.BalanceGroup = (rHcode0246.Sex == Bll.MT.mtEnum.SexCategroy.Both || rHcode0246.Sex == rBase.Sex) ? iBalanceGroup : 0;
                rAbsBalanceRow.RealBalanceGroup = (rHcode0246.Sex == Bll.MT.mtEnum.SexCategroy.Both || rHcode0246.Sex == rBase.Sex) ? iRealBalanceGroup : 0;
        ...
        public List<AbsBalanceRow> GetBalanceNew(string sNobr, DateTime dDate, string sHcode = "", bool bProportion = true, List<BalanceAbsRow> lsAbs = null)
        {
            ...
            foreach (var rHcode in rsHcode)
            {
                ...
                if (rHcode.DCode.Length > 0)
                {
                    ...
                    {
                        case Bll.MT.mtEnum.HcodeUnit.Year:
                        ...
                        case Bll.MT.mtEnum.HcodeUnit.Month:
        ...
        public DataTable GetBalanceView(string sNobr, DateTime dDate, string sNameA = "", DateTime? dDateYB = null, DateTime? dDateYE = null, DateTime? dDateMB = null, DateTime? dDateME = null, bool bProportion = true, List<AbsDataTable> lsAbs = null)
        {
            ...
            foreach (var rBalance in rsBalance)
                if (dt.Columns.Contains(rBalance.Hcode))
                    dt.Columns[rBalance.Hcode].ColumnName = rBalance.HcodeName + "-" + (rBalance.HcodeUnit == Bll.MT.mtEnum.HcodeUnit.Day ? "天" : "時");
        ...
        public int AbsSave(string sNobr, string sHcode, DateTime dDateB, DateTime dDateE, string sTimeB, string sTimeE, string sNote = "", string sKeyMan = "System", string sNameA = "", string sSerno = "", string sAbs1Code = "O", string sDeptsCode = "", bool bCalculateWorkTime = true, bool bCalculateRes = true, decimal iException = 0, bool bFixedCycle = false, string sRoteCode = "")
        {
            ...
            if (rBalance != null && rBalance.lsGuid.Count > 0)
            {
                rsABST = (from c in dcHr.ABS
                          join b in dcHr.HCODE on c.H_CODE equals b.H_CODE
                          where rBalance.lsGuid.Contains(c.Guid)
                          && c.Balance.GetValueOrDefault(0) > 0
                          orderby b.SORT, c.EDATE, c.BDATE
                          select c).ToList();
            }
            ...
            foreach (var rDay in Vdb.Day)
            {
                ...
                if (!rsAbsWhere.Any())
                {
                    ...
                    {
                        var rABS1 = new ABS1();
                        Bll.Tools.DefaultData.SetRowDefaultValue(rABS1);
                        rABS1.NOBR = sNobr;
        ...
        public List<AbsDataTable> GetAbsByDelete(string sNobr)
        {
            ...
            var Vdb1 = (from a in dcHr.ABS
                        ...
                        select new AbsDataTable()
                        {
                            ...
                            Hcode = a.H_CODE.Trim(),
                            HcodeName = h.H_NAME.Trim(),
                            HcodeUnit = h.UNIT.Trim() == "天" ? Bll.MT.mtEnum.HcodeUnit.Day : Bll.MT.mtEnum.HcodeUnit.Hour,
                            Use = a.TOL_HOURS,
                            NameA = a.A_NAME != null ? a.A_NAME.Trim() : "",
                            NoCal = a.nocalc.GetValueOrDefault(false),
                            Serno = a.SERNO.Trim(),
        ...
        public bool AbsCheckInDate(string sNobr, DateTime dDateB, DateTime dDateE, string sHcode)
        {
            bool CanApply = true;

            var rsAbstSql = from a in dcHr.ABS
                            join h in dcHr.HCODE on a.H_CODE equals h.H_CODE
                            where a.BDATE.Date <= dDateB.Date && dDateB.Date <= a.EDATE.Date
                            && h.FLAG == "+"
                            && a.NOBR == sNobr
                            select new BalanceAbstRow
                            {
            ...
            var rsHcode = (from h in dcHr.HCODE
                           join hr in dcHr.HcodeRule on h.DCODE equals hr.Code into hr1
                           from hr1Row in hr1.DefaultIfEmpty()
                           where (from a in rsAbstSql where h.HTYPE == a.HType select 1).Any()
                           && h.FLAG == "-"
            ...
            var DateE = new DateTime(1911, 1, 1);

            var rsAbstWhere = rsAbst.Where(p => p.HType == rsHcode.HType).ToList();

            DateE = rsAbstWhere.Max(p => p.DateE);
