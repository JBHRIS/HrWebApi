...
namespace Portal
{
    public partial class FormAbscChk : WebPageBase
    {
        ...
        {
            var oFormAppAbscDao = new FormsAppAbscByProcessIdDao();
            var FormAppAbscCond = new FormsAppAbscByProcessIdConditions();

            FormAppAbscCond.AccessToken = _User.AccessToken;
            FormAppAbscCond.RefreshToken = _User.RefreshToken;
            FormAppAbscCond.CompanySetting = CompanySetting;
            FormAppAbscCond.ProcessFlowID = lblProcessID.Text;
            FormAppAbscCond.Sign = true;
            FormAppAbscCond.SignState = "";
            FormAppAbscCond.Status = UnobtrusiveSession.Session["RequestName"].ToString() == "View" ? "" : "1";

            var rsFormAppAbsc = oFormAppAbscDao.GetData(FormAppAbscCond);
            var rFormAppAbsc = new FormsAppAbscByProcessIdRow();
            if (rsFormAppAbsc.Status)
            {
                if (rsFormAppAbsc.Data != null)
                {
                    rFormAppAbsc = rsFormAppAbsc.Data as FormsAppAbscByProcessIdRow;
                    if (rFormAppAbsc.FlowApps.Count() != 0)
                    {
                        gvAppS.DataSource = rFormAppAbsc.FlowApps;
        ...
        protected void btnCheck_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var rs = (from c in dcFlow.FormsAppAbsc
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            UnobtrusiveSession.Session["FileTicket"] = (from c in dcFlow.FormsAppAbsc
                                                        where c.AutoKey.ToString() == ca
