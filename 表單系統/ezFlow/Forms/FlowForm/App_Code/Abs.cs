...
public class Abs : System.Web.Services.WebService
{
    public Abs() { }
    ...
    public bool Run(int ID)
    {
        ...
        foreach (var rAppS in dtAppS)
        {
            ...
            if (rAppS.bSign)
            {
                JBHR.Dll.Att.AbsCal.AbsSave(rAppS.sNobr, rAppS.sHcode, rAppS.dDateB, rAppS.dDateE, rAppS.sTimeB, rAppS.sTimeE, "", rAppS.sNote, rAppM.sName + "e", rAppS.sReserve2, 0, Guid.NewGuid().ToString());
                rAppS.sState = "3";

                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之請假單(已核准)";
            }
            else
            {
                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之請假單(已退簽)";
            }
