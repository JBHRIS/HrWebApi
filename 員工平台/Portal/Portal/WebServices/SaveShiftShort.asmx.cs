...
namespace Portal.WebServices
{
    ...
    public class SaveShiftShort : System.Web.Services.WebService
    {
        ...
        public bool Run(int ID)
        {
            ...
            OldDal.Dao.Att.RotechgDao oRotechgDao = new OldDal.Dao.Att.RotechgDao(dcHR.Connection);
            ...
            foreach (var rAppS in rsAppS)
            {
                ...
                if (rAppS.Sign)
                {
                    bool bPass = oRotechgDao.Save(rAppS.EmpId, rAppS.DateB, rAppS.RoteCodeE, "", rAppM.EmpName, rAppM.ProcessID);
                    bPass = oRotechgDao.Save(rAppS.EmpId, rAppS.DateE, rAppS.RoteCodeB, "", rAppM.EmpName, rAppM.ProcessID);
                    if (bPass)
                    {
                        OldDal.Dao.Att.TransCardDao oTransCardDao = new OldDal.Dao.Att.TransCardDao(dcHR.Connection);
                        oTransCardDao.TransCard(rAppS.EmpId, rAppS.EmpId, "0", "z", rAppS.DateB, rAppS.DateB, rAppS.EmpId, true, true, true, "", "JB-TRANSCARD", true, 1);
                    }
                    rAppS.SignState = "3";
                }
