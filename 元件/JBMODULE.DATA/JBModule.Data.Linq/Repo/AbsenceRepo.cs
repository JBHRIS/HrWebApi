...
namespace JBModule.Data.Repo
{
    public class AbsenceRepo
    {
        ...
        public static List<Dto.AbsenceDto> GetAbsTakenByByEmployeeIdList(DateTime DateBegin, DateTime DateEnd, bool WithFlowData, List<string> EmployeeIdList, List<string> HtypeList = null)
        {
            ...
            List<Dto.AbsenceDto> sql = new List<Dto.AbsenceDto>();
            foreach (var item in EmployeeIdList.Split(1000))
            {
                var sql1 = (from a in db.ABS
                           ...
                           select new Dto.AbsenceDto { Adate = a.BDATE.Date, BeginTime = a.BDATE.Date.AddTime(a.BTIME), EndTime = a.BDATE.Date.AddTime(a.ETIME), Guid = a.Guid, Hcode = a.H_CODE, Memo = a.NOTE, Nobr = a.NOBR, Serno = a.SERNO, Taken = a.TOL_HOURS, Hname = c.H_NAME, Unit = c.UNIT, NameC = d.NAME_C, HcodeDisp = c.H_CODE_DISP }).ToList();
                sql.AddRange(sql1);
            }
            return sql;
        ...
        public static List<Dto.AbsenceDetail> GetAbsEntitleByEmployeeIdList(DateTime DateBegin, DateTime DateEnd, bool WithFlowData, List<string> EmployeeIdList = null, List<string> HtypeList = null)
        {
            ...
            if (HtypeList == null)
                HtypeList = db.HcodeType.Select(p => p.HTYPE).ToList();
            List<Dto.AbsenceDetail> sql = new List<Dto.AbsenceDetail>();
            foreach (var item in EmployeeIdList.Split(1000))
            {
                var sql1 = (from a in db.ABS
                          join c in db.HCODE on a.H_CODE equals c.H_CODE
                          ...
                          && item.Contains(a.NOBR)
                           && (HtypeList == null || HtypeList.Contains(c.HTYPE))
                           && DateBegin <= a.EDATE && DateEnd >= a.BDATE
                          select new Dto.AbsenceDetail { Balance = a.Balance.GetValueOrDefault(0), BeginDate = a.BDATE, EndDate = a.EDATE, Entitled = a.TOL_HOURS, HCode = c.H_CODE, HCodeDisp = c.H_CODE_DISP, HName = c.H_NAME, NameC = d.NAME_C, Nobr = a.NOBR, Taken = a.LeaveHours.GetValueOrDefault(0), Unit = c.UNIT, Guid = a.Guid }).ToList();
                sql.AddRange(sql1);
            }
            return sql;
        }
