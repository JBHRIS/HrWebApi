...
namespace Portal
{
    public partial class FormOt1Std : WebPageBase
    {
        private dcHrDataContext dcHR = new dcHrDataContext();

        private dcFlowDataContext dcFlow = new dcFlowDataContext();
        private string _FormCode = "Ot1";
        ...
        private void SetTotalOtHours(string sNobr, DateTime dDate)
        {
            ...
            var rsAppS = (from c in dcFlow.wfAppOt
                          where ((c.sProcessID == lblProcessID.Text && c.sState == "1") || (c.idProcess != 0 && c.sState == "1"))
                          && c.sNobr == sNobr
                          select c).ToList();

            var rsFlow = rsAppS.Where(p => TwoDate.DateA <= p.dDateB && p.dDateB <= TwoDate.DateD).ToList();
            if (rsFlow.Count > 0)
                OtFlowHours += rsFlow.Sum(p => p.iTotalHour); //加班時數
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (DateTimeB.Date < DateTime.Now.Date)
            {
                ...
                var rAbs = rsAbs.Where(p => p.Hcode == "N").FirstOrDefault();
                bool IsAbs = false;
                if (rAbs != null)
                {
            ...
            var rsFormsAppInfo = new FormsAppInfo()
            {
                ...
                InfoMail = MessageSendMail.Ot1Body(rsFormsAppOt.EmpId, rsFormsAppOt.EmpName, rsFormsAppOt.DeptName, RoteName, rsFormsAppOt.DateB, rsFormsAppOt.TimeB, rsFormsAppOt.TimeE, rsFormsAppOt.Use, rsFormsAppOt.OtCateName, rsFormsAppOt.Note)
            };
