...
namespace ezEngineServices
{
    public class CFlowManage
    {
        ...
        public CFlowManage(IDbConnection conn)
        {
        ...
        public CFlowManage(string ConnectionString)
        {
        ...
        /// </summary>
        /// <param name="ConnectionString"></param>
        public CFlowManage(dcFlowDataContext dcFlow)
        {
            _ConnectionString = dcFlow.Connection.ConnectionString;
            this.dcFlow = dcFlow;
        }
        ...
        public List<int> FlowResubmit(List<int> lsProcessID, bool bPreviousStep = false, string idEmp_Agent = "")
        {
            ...
            {
                var rEmpAgent = (from c in dcFlow.Role
                    ...
                    if (rFormApp != null)
                    {
                        ...
                        if (!bReSend)
                        {
                            ...
                            if (rsRoleWhere.Count > 0)
                            {
                                ...
                                dcFlow.wfFormSignM.DeleteAllOnSubmit(rsFormSignM_Where);
            ...
            dcFlow.ProcessApParm.DeleteAllOnSubmit(lsDeleteProcessApParm);
            dcFlow.ProcessApView.DeleteAllOnSubmit(lsDeleteProcessApView);
            dcFlow.ProcessCheck.DeleteAllOnSubmit(lsDeleteProcessCheck);
            dcFlow.ProcessFlow.DeleteAllOnSubmit(lsDeleteProcessFlow);
            dcFlow.ProcessNode.DeleteAllOnSubmit(lsDeleteProcessNode);
            ...
            foreach (var rFlowResubmitRow in lsFlowResubmitRow)
            {
                try
                {
                    ...
                    if (rForm != null && rForm.sTableName.Length > 0)
                        dcFlow.ExecuteCommand("Update " + rForm.sTableName + " Set sState = '1' , bSign = '1' Where idProcess = " + rFlowResubmitRow.idProcess);
        ...
        public List<int> FlowStateSet(List<int> lsProcessID, FlowState State, string idEmp = "")
        {
            ...
            foreach (var rProcessFlow in rsProcessFlow)
            {
                ...
                if (rForm != null)
                {
                    ...
                    if (rForm.sTableName.Length > 0)
                        if (State == FlowState.Delete)//刪除實體資料
                            dcFlow.ExecuteCommand("Delete From " + rForm.sTableName + " Where idProcess = " + rProcessFlow.id);
                        else
                        {
                            dcFlow.ExecuteCommand("Update " + rForm.sTableName + " Set sState = '" + sState + "' , bSign = " + sSign + " Where idProcess = " + rProcessFlow.id);
                            ...
                            //子流程抽單 20200412 by ming
                            if (State == FlowState.Take)
                            {
                                var rsProcessMultiFlow = (from c in dcFlow.ProcessMultiFlow
                                                          where c.ProcessFlow_id == rProcessFlow.id
                                                          select c).ToList();

                                var ListProcessMultiFlowId = rsProcessMultiFlow.Select(p => p.SubProcessFlow_id.GetValueOrDefault(0)).ToList();
                                FlowStateSet(ListProcessMultiFlowId, State, idEmp);
                            }
                        }
                ...
                if (State != FlowState.Delete)
                {
                    ...
                    dcFlow.wfFormSignM.InsertOnSubmit(rFormSignM);
            ...
            if (State == FlowState.Delete)
            {
                dcFlow.wfFormSignM.DeleteAllOnSubmit(rsFormSignM);
                dcFlow.wfFormAppInfo.DeleteAllOnSubmit(rsFormAppInfo);
                dcFlow.wfFormApp.DeleteAllOnSubmit(rsFormApp);

                dcFlow.ProcessApView.DeleteAllOnSubmit(rsProcessApView);
                dcFlow.ProcessApParm.DeleteAllOnSubmit(rsProcessApParm);
                dcFlow.ProcessFlowShare.DeleteAllOnSubmit(rsProcessFlowShare);
                dcFlow.ProcessCheck.DeleteAllOnSubmit(rsProcessCheck);
                dcFlow.ProcessNode.DeleteAllOnSubmit(rsProcessNode);
                dcFlow.ProcessFlow.DeleteAllOnSubmit(rsProcessFlow);
            }
        ...
        public void FlowSign(List<int> lsProcessID, string idEmp = "")
        {
            ...
            var rsFormApp = (from fa in dcFlow.wfFormApp
            ...
            foreach (var rProcessFlow in rsProcessFlow)
            {
                if (rEmpM != null)
                {
                    ...
                    if (rFormApp != null)
                    {
                        ...
                        if (rForm != null)
                            dcFlow.ExecuteCommand("Update " + rForm.sTableName + " Set bSign = {0} , sState = {1} Where idProcess = {2}", true, rFormApp.sState, rProcessFlow.id);
                    }
                }
            }
        ...
        public List<int> FlowSignWorkFinish(List<int> lsProcessID, string idEmp = "", bool bSign = true, string sNote = "", bool EmpSameUp = true)
        {
            ...
            foreach (var rProcessFlow in rsProcessFlow)
            {
                ...
                if (rProcessNode != null)
                {
                    ...
                    if (rProcessCheck != null)
                    {
                        var rEmpSign = (from role in dcFlow.Role
                                        join dept in dcFlow.Dept on role.Dept_id equals dept.id
                        ...
                        if (rProcessApParm != null)
                        {
                            if (rEmpM != null)
                            {
                                ...
                                if (rFormApp != null)
                                {
                                    ...
                                    if (rForm != null)
                                        dcFlow.ExecuteCommand("Update " + rForm.sTableName + " Set bSign = {0} , sState = {1} Where idProcess = {2}", bSign, rFormApp.sState, rProcessFlow.id);
                                }
                            }
        ...
        {
            var Vdb = (from pn in dcFlow.ProcessNode
                       join pc in dcFlow.ProcessCheck on pn.auto equals pc.ProcessNode_auto into pn1
                       ...
                       join e1 in dcFlow.Emp on pn1Row.Emp_idDefault equals e1.id into pc1
                       from pc1Row in pc1.DefaultIfEmpty()
                       join e2 in dcFlow.Emp on pn1Row.Emp_idAgent equals e2.id into pc2
                    ...
                    foreach (var rNode in rsProcessNode)
                    {
                        ...
                        if (rNode.auto == rProcessNode.auto)
                        {
                            ...
                            if (rNode.isMulti.Value)
                            {
                                foreach (var rProcessMultiFlow in rsProcessMultiFlow)
                                {
                                    var rSubProcessNode = (from c in dcFlow.ProcessNode
                        ...
                        {
                            //其它節點 則全部刪除
                            dcFlow.ProcessNode.DeleteOnSubmit(rNode);
        ...
        public List<int> FlowStateSetByRWD(List<int> lsProcessID, FlowState State, string idEmp = "")
        {
            ...
            var rsFormApp = (from pa in dcFlow.FormsApp
                             where (from pf in rsProcessFlowSql where pa.idProcess == pf.id select 1).Any()
                             select pa).ToList();

            var rsFormAppInfo = (from f in dcFlow.FormsAppInfo
                                 where (from pf in rsProcessFlowSql where f.idProcess == pf.id select 1).Any()
                                 select f).ToList();

            var rsFormSignM = (from f in dcFlow.FormsSign
                               where (from pf in rsProcessFlowSql where f.idProcess == pf.id select 1).Any()
                               select f).ToList();
            ...
            foreach (var rProcessFlow in rsProcessFlow)
            {
                ...
                if (rForm != null)
                {
                    ...
                    if (rForm.TableName.Length > 0)
                        if (State == FlowState.Delete)//刪除實體資料
                            dcFlow.ExecuteCommand("Delete From " + rForm.TableName + " Where idProcess = " + rProcessFlow.id);
                        else
                        {
                            dcFlow.ExecuteCommand("Update " + rForm.TableName + " Set SignState = '" + sState + "' , Sign = " + sSign + " Where idProcess = " + rProcessFlow.id);
                    ...
                    if (rForm.SaveMethod.Length > 0 && (State == FlowState.Approve || State == FlowState.Reject))
                    {
                        ...
                        if (true)
                        {
                            string Metod = rForm.SaveMethod.Length > 0 ? rForm.SaveMethod : "Run";
                ...
                {
                    //寫入簽核資料
                    var rFormSignM = new FormsSign();
                    rFormSignM.idProcess = rProcessFlow.id;
                    rFormSignM.FormsCode = sFormCode;
                    rFormSignM.ProcessId = rFormSignM.idProcess.ToString();
                    rFormSignM.EmpId = idEmp;
                    rFormSignM.EmpName = "Sys";
                    rFormSignM.DeptName = "Top";
                    if (rEmpM != null)
                    {
                    ...
                    rFormSignM.Key2 = "";
                    rFormSignM.Sort = 1;
                    rFormSignM.SignNote = State.ToString();
                    rFormSignM.Sign = sState == "3";
                    rFormSignM.InsertDate = DateTime.Now;
                    rFormSignM.Status = "1";
                    dcFlow.FormsSign.InsertOnSubmit(rFormSignM);
            ...
            if (State == FlowState.Delete)
            {
                dcFlow.FormsSign.DeleteAllOnSubmit(rsFormSignM);
                dcFlow.FormsAppInfo.DeleteAllOnSubmit(rsFormAppInfo);
                dcFlow.FormsApp.DeleteAllOnSubmit(rsFormApp);
        ...
        public List<int> FlowResubmitByRWD(List<int> lsProcessID, bool bPreviousStep = false, string idEmp_Agent = "")
        {
                    ...
                    if (rFormApp != null)
                    {
                        ...
                        if (!bReSend)
                        {
                            ...
                            if (rsRoleWhere.Count > 0)
                            {
                                ...
                                //刪除所有簽核資料
                                var rsFormSignM_Where = rsFormSignM.Where(p => p.idProcess == rProcessFlow.id).ToList();
                                dcFlow.FormsSign.DeleteAllOnSubmit(rsFormSignM_Where);
            ...
            foreach (var rFlowResubmitRow in lsFlowResubmitRow)
            {
                try
                {
                    ...
                    if (rForm != null && rForm.TableName.Length > 0)
                        dcFlow.ExecuteCommand("Update " + rForm.TableName + " Set SignState = '1' , Sign = '1' Where idProcess = " + rFlowResubmitRow.idProcess);
        ...
        public List<int> FlowProcessRoleFix(List<int> lsProcessID)
        {
            ...
            foreach (var rProcessFlow in rsProcessFlow)
            {
                var rProcessRole = (from role in dcFlow.Role
                                    join emp in dcFlow.Emp on role.Emp_id equals emp.id
                                    join dept in dcFlow.Dept on role.Dept_id equals dept.id
                                    join pos in dcFlow.Pos on role.Pos_id equals pos.id
                                    where role.Emp_id == rProcessFlow.Emp_id
                                    select new
                                    {
                                        RoleId = role.id,
                                        EmpNobr = emp.id,
                                        ...
                                        Auth = role.deptMg.Value,
                                        Sort = role.sort
                                    }).OrderBy(p => p.Sort).FirstOrDefault();

                if (rProcessRole != null)
                {
                    rProcessFlow.Role_id = rProcessRole.RoleId;
                    rsProcessID.Add(rProcessFlow.id);
                }
