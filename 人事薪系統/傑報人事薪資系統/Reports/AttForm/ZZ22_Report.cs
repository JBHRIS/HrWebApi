 ...
 * 功能代號：ZZ22
 * 功能路徑：報表列印 > 出勤 > 加班報表
 * 檔案路徑：~\Customer\JBHR2\人事薪系統\傑報人事薪資系統\Reports\AttForm\ZZ22_Report.cs
 * 功能用途：
 *  
 * 
 * 版本記錄：
 ...
 *    日期           人員           版本           說明
 * ------------------------------------------------------------------------------------------------------
 * 2021/04/22    Daniel Chih    Ver 1.0.01     1. 修改AB帳中，B帳的加班資料Table從ot改為ot_b
 * 2021/04/22    Daniel Chih    Ver 1.0.02     1. 增加伙食費薪資數值解密的語法
 * 2021/06/03    Daniel Chih    Ver 1.0.03     1. 修改伙食費解密的方式，避開薪資金鑰
 * 
 * 
 * ======================================================================================================
 * 
 * 最後修改：Daniel Chih (0492) - 2021/06/03
 */
    ...
    {      
        ...
        private void ZZ22_Report_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                rq_base.Columns.Add("otminhrs", typeof(decimal));
                rq_base.PrimaryKey = new DataColumn[] { rq_base.Columns["nobr"] };
                foreach (DataRow Row in rq_base.Rows)
                {
                    Row["otmaxhrs"] = otmaxhr;
                    Row["otminhrs"] = otminhr;
                }
                rq_sys3 = null;
                ...
                //if (labcheck)
                //{
                //    CmdOt += "datename(dw,a.bdate) as dw,c.rote_disp,e.d_no_disp as otdept,e.d_name as otdname,d.rote";
                //    CmdOt += " from attend d, ot a";
                //    CmdOt += " left outer join otrcd b on a.otrcd=b.otrcd";
                //    CmdOt += " left outer join rote c on a.ot_rote=c.rote";
                //    CmdOt += " left outer join depts e on a.ot_dept=e.d_no";
                //    CmdOt += " where a.nobr=d.nobr and a.bdate=d.adate";
                //    CmdOt += string.Format(@" and a.nobr between '{0}' and '{1}'", nobr_b, nobr_e);

                //}
                //else
                //{
                //    CmdOt += "datename(dw,a.bdate) as dw,c.rote_disp,e.d_no_disp as otdept,e.d_name as otdname";
                //    CmdOt += " from ot a  left outer join otrcd b on a.otrcd=b.otrcd";
                //    CmdOt += " left outer join rote c on a.ot_rote=c.rote";
                //    CmdOt += " left outer join depts e on a.ot_dept=e.d_no";
                //    CmdOt += string.Format(@" where a.nobr between '{0}' and '{1}'", nobr_b, nobr_e);
                //}

                //2014/01/24新增出勤班別篩選條件
                CmdOt += "datename(dw,a.bdate) as dw,c.rote_disp,e.d_no_disp as otdept,e.d_name as otdname,d.rote";
                CmdOt += string.Format(@",{0} as maxhrs", otmaxhr);
                if (labcheck)
                {
                    //將來源table的ot改成ot_b
                    CmdOt += " from attend d inner join ot_b a on a.nobr=d.nobr and a.bdate=d.adate";
                }
                else
                {
                    CmdOt += " from attend d inner join ot a on a.nobr=d.nobr and a.bdate=d.adate";
                }
                CmdOt += string.Format(@" and a.nobr between '{0}' and '{1}'", nobr_b, nobr_e);
                if (date_type == "1") 
                    CmdOt += string.Format(@" and a.yymm between '{0}' and '{1}'", yymm_b, yymm_e);
                if (date_type == "2") 
                ...
                CmdOt += " left outer join depts e on a.ot_dept=e.d_no";
                CmdOt += " where 1 = 1 ";
                //CmdOt += string.Format(@" and c.rote between '{0}' and '{1}'", rote_b, rote_e);
                if(reporttype=="4")   CmdOt += noen;
                CmdOt += " order by a.nobr,a.bdate";
                DataTable rq_ot = SqlConn.GetDataTable(CmdOt);

                string attdate_b = date_b; string attdate_e = date_e;
                if (date_type == "1")
                {
                    //DataRow[] Mindate = rq_ot.Select("", "bdate asc");
                    //DataRow[] Maxdate = rq_ot.Select("", "bdate desc");
                    //attdate_b = DateTime.Parse(Mindate[0]["bdate"].ToString()).ToString("yyyy/MM/dd");
                    //attdate_e = DateTime.Parse(Maxdate[0]["bdate"].ToString()).ToString("yyyy/MM/dd");
                    attdate_b = JBHR.Reports.ReportClass.GetAttBDate(yymm_b.Substring(0, 4), yymm_b.Substring(4, 2));
                    attdate_e = JBHR.Reports.ReportClass.GetAttEDate(yymm_b.Substring(0, 4), yymm_b.Substring(4, 2));
                }
                if (labcheck)
                {

                    JBHR.Reports.AttForm.ACCS.ACCB ABDT = new AttForm.ACCS.ACCB();
                    ABDT.GetACCB(rq_ot, Convert.ToInt32(otminhr), Convert.ToInt32(otmaxhr), DateTime.Parse(attdate_b), DateTime.Parse(attdate_e));
                }
                ...
                foreach (DataRow Row in rq_ot.Rows)
                {
                    ...
                    if (labcheck)
                    {
                        ...
                        if (row1 != null)
                        {
                            if (decimal.Parse(Row["ot_hrs"].ToString()) <= 0)
                                Row.Delete();
                        }
                        else
                            Row.Delete();
                ...
                foreach (DataRow row4 in rq_ot.Rows)
                {
                    ...
                    if (row5 != null)
                    {
                        ...
                        //aRow["ot_foodh"] = JBModule.Data.CDecryp.Number(decimal.Round(decimal.Parse(row4["ot_foodh"].ToString()), 0));
                        aRow["ot_foodh"] = JBTools.Security.SalaryEncrypt.SalaryDecode(decimal.Round(decimal.Parse(row4["ot_foodh"].ToString()), 0));
                        aRow["d_name"] = row5["d_name"].ToString();
                        aRow["d_ename"] = row5["d_ename"].ToString();
                        aRow["otrname"] = row4["otrname"].ToString();
                        aRow["ot_dept"] = row4["otdept"].ToString();
                        aRow["otdname"] = row4["otdname"].ToString();
                        aRow["note"] = row4["note"].ToString();
                        //if (row6 != null)
                        //{
            ...
            switch (reporttype)
            {
                case "0":
                    ...
                    foreach (DataRow Row in DTrow)
                    {
                        ...
                        aRow["伙食費"] = int.Parse(Row["ot_foodh"].ToString());
                    ...
                    break;
                case "1":
                    DTrow = DT.Select("", "ot_dept,nobr asc");
                    ...
                    foreach (DataRow Row1 in DTrow)
                    {
                        ...
                        _value[0] = Row1["ot_dept"].ToString();
                        ...
                        {
                            DataRow aRow = ExporDt.NewRow();
                            aRow["成本部門代碼"] = Row1["ot_dept"].ToString();
                            aRow["成本部門名稱"] = Row1["otdname"].ToString();                            
