...
namespace JBHR.Sal
{
    public class BaseValue
    {
        ...
        public static string MinNobr
        {
            ...
            {
                SalaryMDDataContext smd = new SalaryMDDataContext();
                var sql = from a in smd.BASE where  smd.GetFilterByNobr(a.NOBR, MainForm.USER_ID, MainForm.COMPANY, MainForm.ADMIN).Value  select a;
                string min = sql.Min(p => p.NOBR);
                return min;
            }
        }
        /// <summary>
