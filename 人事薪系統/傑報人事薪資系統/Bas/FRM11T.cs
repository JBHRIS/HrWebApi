...
namespace JBHR.Bas
{
    public partial class FRM11T : JBControls.JBForm
    {
        public FRM11T()
        {
            InitializeComponent();
        ...
        private void FRM11M_Load(object sender, EventArgs e)
        {
            ...
            fullDataCtrl1.DataAdapter = outPostTableAdapter;
            ...
            fullDataCtrl1.CodeColumn = "OutPost.Code";//**代碼權限設定**
            fullDataCtrl1.CodeSource = "OutPost";//**代碼權限設定**
            fullDataCtrl1.DataAdapter = outPostTableAdapter;
        ...
        private void btnCodeGroup_Click(object sender, EventArgs e)
        {
            ...
            {
                if (outPostBindingSource.Current == null) return;
                BasDS.OutPostRow r = ((outPostBindingSource.Current as DataRowView).Row as BasDS.OutPostRow);
                if (r != null)//**代碼權限設定**20121114
                {
                    Sys.SYS_CODE frm = new Sys.SYS_CODE();
                    frm.Source = "OutPost";
                    frm.Code = r.Code;
                    frm.Text += "(" + r.OutPostName + ")";
                    frm.ShowDialog();
                }
            }
        }
        ...
        bool CheckRepeat(string Code, string Disp)//**代碼權限設定**20121114
        {
            ...
            var sql = from a in db.OutPost
                     ...
                     && db.GetCodeFilter("OutPost", a.Code, MainForm.USER_ID, MainForm.COMPANY, true).Value//管理者權限設定true,才可以抓到該公司所有代碼
                      select a;
            return sql.Any();
        }
    }
