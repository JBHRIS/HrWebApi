...
namespace JBHRIS.Api.Dal.ezFlow.ezEngineServicesImplement
{
    ...
    /// FlowStart  有重複注入 
    /// </summary>
    public class CFlow : ICFlowInterface
    {
        ...
        public bool isFinishOK { get; set; }

        public CFlow(ezFlowContext context,
                    //IServiceInterface serviceInterface,
                    ICProcess_Dal cProcessInterface ,
                    ICOrg_Dal cOrgInterface)
        {
        ...
        public List<string> GetLinkNextNode(int idProcess, int idProcessNode_Source, int idProcessCheck_Source, string idFlowTree, string idFlowNode_Source)
        {
            ...
            var rsFlowLinkSql = from c in this._context.FlowLinks
                                where c.FlowTree_id == idFlowTree
                                && c.FlowNode_idSource == idFlowNode_Source
                                select c;

            var rsFlowLinkPower = (from c in this._context.FlowLinkPowers
            ...
            foreach (var rFlowLink in rsFlowLink)
            {
                ...
                if (rFlowLink.linkType == "1")  //有條件的緣色線 只要有符合都要進去
                {
                    ...
                    {
                        string Where = "";
                        string SqlCommandText = "Select * From " + rFlowLinkPowerWhere.tableName
                        ...
                        try
                        {
                            results = this._context.wfFormApps.FromSqlRaw(SqlCommandText).Count();
                        }
                        catch (Exception ex)
                        {
                            this._ICProcessInterface.WriteProcessException(idProcess, idProcessNode_Source, idProcessCheck_Source,MsgType.Error, "錯誤：" + ex.Message);
                        }
        ...
        /// <returns></returns>
        public bool GoToNextNode(int idProcess, int idProcessNode_Source, int idProcessCheck_Source, string idFlowTree, string idFlowNodeSource, string idRoleSource, string idEmpSource, List<string> lstNode_Next, bool EmpSameUp = true, CMan Man_Default = null, CMan Man_Agent = null, bool FlowStart = false)
        {
            ...
            //if (rProcessFlow != null)
            {
                foreach (string sNode_Next in lstNode_Next)
                {
                    ...
                    switch (rFlowNode.nodeType)
                    {
                        ...
                        case "3": //主管審核
                            ...
                            if (Man_Default == null)
                            {
                                if (bNextManage)
                                {
                                    ...
                                    Man_Default = this._ICOrgInterface.GetManager(idProcess, idRoleSource, EmpSameUp);

                                    if (Man_Default != null)
                                    {
                                        //檢驗主官是否為該部門的主管 下一關的部門path應該被上一關的部門path包含
                                        if (!this._ICOrgInterface.IsDeptPathTrue(idRoleSource, Man_Default.idRole))
                                        {
                                            this._ICProcessInterface.WriteProcessException(idProcess, idProcessNode_Source, idProcessCheck_Source,
                                                      MsgType.Error, "錯誤：主官審核此審核關卡並非真正的下一關");
                                            isOK = false;
                                            break;
                                        }

                                        if (!this._ICOrgInterface.IsManage(Man_Default.idRole, Man_Default.idEmp))
                                        {
                                            this._ICProcessInterface.WriteProcessException(idProcess, idProcessNode_Source, idProcessCheck_Source,
                            ...
                            if (Man_Default == null)
                            {
                                ...
                                this._ICProcessInterface.WriteProcessNodeAndCheck(idProcessNode_Source, idProcess, sNode_Next, Man_Default, Man_Agent, idEmpSource);
                            break;
                        case "4": //流程起始者
                            if (Man_Default == null)
                                Man_Default = this._ICOrgInterface.GetFlowInit(idProcess);
                            ...
                            else
                            {
                                ...
                                {
                                    using (var transaction1 = this._context.Database.BeginTransaction())
                                    {
                                        try
                                        {
                                            int ProcessApParmAuto = this._ICProcessInterface.WriteProcessNodeAndCheck(idProcessNode_Source, idProcess, sNode_Next, Man_Default, Man_Agent, idEmpSource);
                                            //&& p.isNextNodeAutoSign.Value 缺少欄位
                                            var rNodeDynamic = this._context.NodeInits.FirstOrDefault(p => p.FlowNode_id == sNode_Next);
                                            ...
                                            if (rNodeDynamic != null)
                                            {
                                                ...
                                                //重複注入
                                                //this._IServiceInterface.WorkFinish(ProcessApParmAuto);
                                            }
                        ...
                        case "5": //會簽起始者
                            if (Man_Default == null)
                                Man_Default = this._ICOrgInterface.GetMultiInit(idProcess, sNode_Next);
                        ...
                        case "6": //自訂簽核者
                            if (Man_Default == null)
                                Man_Default = this._ICOrgInterface.GetCustom(sNode_Next);
                        ...
                        case "7": //動態簽核者
                            if (Man_Default == null)
                                Man_Default = this._ICOrgInterface.GetDynamic(idProcess, sNode_Next);
                                ...
                                else
                                {
                                    using (var transaction1 = this._context.Database.BeginTransaction())
                                    {
                                        try
                                        {
                                            ...
                                            //&& p.isNextNodeAutoSign.Value 缺少欄位
                                            var rNodeDynamic = this._context.NodeDynamics.FirstOrDefault(p => p.FlowNode_id == sNode_Next);
                        ...
                        case "8": //代理起始者
                            if (Man_Default == null)
                                Man_Default = this._ICOrgInterface.GetAgentInit(idProcess);
                        ...
                        case "9": //會簽流程
                            {
                                ...
                                int ProcessNode_auto = this._ICProcessInterface.CreateProcessNode(idProcessNode_Source, idProcess, sNode_Next);

                                var rsProcessMultiFlowSql = from c in this._context.ProcessMultiFlows
                                                            where c.ProcessNode_auto == 0
                                                            && c.ProcessFlow_id == idProcess
                                                            select c;

                                var rsFlowNodeSql = from c in this._context.FlowNodes
                                ...
                                foreach (var rProcessMultiFlow in rsProcessMultiFlow)
                                {
                                    ...
                                    int sub_idProcess = this._ICProcessInterface.GetProcessID(); //先取得 idProcess
                                    ...
                                    if (hasStart)
                                    {
                                        ...
                                        if (rFlowLink != null)
                                        {
                                            ...
                                            rFlowNode = (from c in this._context.FlowNodes
                                    ...
                                    {	//填完動態成員資訊後，啟動子流程，並設定為並行。
                                        //重複注入
                                        //this._IServiceInterface.FlowStart(sub_idProcess, rProcessMultiFlow.SubFlowTree_id, rProcessMultiFlow.SubInitRole_id, rProcessMultiFlow.SubInitEmp_id ,"","");
                        ...
                        case "11": //服務程式，直接過去吧！
                            ...
                            var rNodeService = (from c in this._context.NodeServices
                        ...
                        case "12": //流程結束
                            {
                                ...
                                if (rProcessMultiFlow != null)
                                {
                                    ...
                                    var rsProcessFlow = (from c in this._context.ProcessFlows
                                    ...
                                    if (isAllFinish)
                                    {
                                        rProcessFlow = (from c in this._context.ProcessFlows
        ...
        public bool IsNextManage(int idProcess, int idProcessNode_Source, int idProcessCheck_Source, string idFlowNode)
        {
            ...
            var r1 = (from c in this._context.ProcessNodes
                      join n in this._context.FlowNodes on c.FlowNode_id equals n.id
            ...
            if (r1 != null && r1.nodeType != "3")
            {
                ...
                {
                    var r2 = (from c in this._context.ProcessNodes
                              join n in this._context.FlowNodes on c.FlowNode_id equals n.id
            ...
            var rsNodeMangLoopBreak = (from c in this._context.NodeMangLoopBreaks
            ...
            if (rsNodeMangLoopBreak.Count > 0)
            {
                ...
                foreach (var rNodeMangLoopBreak in rsNodeMangLoopBreak)
                {
                    ...
                    string Where = "";
                    string SqlCommandText = "Select * From " + rNodeMangLoopBreak.tableName
                        + " Where idProcess = " + idProcess.ToString();
