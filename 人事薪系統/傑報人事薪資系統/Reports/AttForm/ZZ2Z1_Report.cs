...
namespace JBHR.Reports.AttForm
{
    public partial class ZZ2Z1_Report : JBControls.JBForm
    {
        ...
        bool exportexcel, No_Dept_Sum, checkBPBF;
        public ZZ2Z1_Report(string nobrb, string nobre, string deptb, string depte, string deptsb, string deptse, string joblb, string joble
            , string dateb, string datee, string yymmb, string yymme, string empb, string empe, string compb, string compe
            , string workb, string worke, string responsibilityb, string responsibilitye, string datareport, bool _exportexcel
            , bool _No_Dept_Sum, string compname, string _CompId, bool check_BPBF)
        {
            InitializeComponent();
            ...
            No_Dept_Sum = _No_Dept_Sum; checkBPBF = check_BPBF;
        }
        ...
        private void ZZ2Z1_Report_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                CalcSalaryByAttFunction CalAttFull = new CalcSalaryByAttFunction();
                Dictionary<string, decimal> FullAttAmt = new Dictionary<string, decimal>();
                ...
                foreach (DataRow Row in ds.Tables["zz2z1"].Rows)
                {
                    ...
                    if (row3 != null)
                    {
                        ...
                        if (checkBPBF)
                        {
                            Row["att_amt"] = FullAttAmt[str_nobr];
                        }
                        else
                        {
                ...
                {                    
                    ...
                    if (No_Dept_Sum)
                    {
                        RptViewer.LocalReport.ReportPath = _rptpath + "Rpt_zz2z10_No_Dept_Sum.rdlc";
                    }
                    else
                    {
                        //判斷是否為寶齡獨有報表
