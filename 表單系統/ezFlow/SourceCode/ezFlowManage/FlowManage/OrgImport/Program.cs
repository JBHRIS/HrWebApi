...
namespace OrgImport
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            TextLog.WriteLog("匯入版本：20130826");
            ...
            var rsHrBase = (from b in dcHr.BASE
                            ...
                            on b.NOBR.Trim() equals bt.NOBR.Trim()
                            join d in dcHr.DEPT
                            on bt.DEPT.Trim() equals d.D_NO.Trim()
                            join dm in dcHr.DEPTA
                            on bt.DEPTM.Trim() equals dm.D_NO.Trim()
                            join p in dcHr.JOB
                            on bt.JOB.Trim() equals p.JOB1.Trim()
                            where bt.ADATE.Date <= dDate
            ...
            var rsHrJob = (from c in dcHr.JOB
                           ...
                           select new
                           {
                               ...
                               Tree = c.JOB_TREE != null ? c.JOB_TREE.Trim() : "",
                           }).ToList();
            ...
            foreach (var rHrJob in rsHrJob)
            {
                if (rHrJob.Code.Length > 0)
                {
                    ...
                    rPos.PosLevel_id = rHrJob.Tree.Length > 0 ? rHrJob.Tree : "0";
                    lsPos.Add(rPos);
            ...
            var rsHrDeptmWhere = rsHrDeptm.Where(p => p.ManageNobr.Length > 0);
