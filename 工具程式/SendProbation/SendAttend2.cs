...
namespace sendAttendMail
{
    class SendAttend2
    {
        ...
        public static void DoSend()
        {
            ...
            Dt.Columns.Add("合同起始日", typeof(string));
            Dt.Columns.Add("合同到期日", typeof(string));   
            ...
            try
            {
                ...
                string Cmdparameter = "select code,value from Parameter where code in ('JbMail.DateInteval','JbMail.Sender','JbMail.TestAccount','JbMail.SenderName','JbMail.Department','JbMail.HRAccount','JbMail.ContractDays')";
                ...
                foreach (DataRow Row in rq_parameter.Rows)
                {
                    ...
                    else if (Row["code"].ToString().Trim() == "JbMail.ContractDays")
                ...
                {
                    //Row["datagroup"] = "';delete from dbo.Test--";
                    //產生合同到職通知
                    string CmdBase = "select c.compname,b.nobr,a.name_c,a.name_e,b.indt,f.adate,f.ddate,d.d_no_disp as dept,d.d_name";
                    CmdBase += ",b.indt,b.workcd,g.work_addr";
                    CmdBase += " from contract f ,base a,basetts b";
                    CmdBase += " left outer join comp c on b.comp=c.comp";
                    CmdBase += " left outer join dept d on b.dept=d.d_no";                    
                    CmdBase += " left outer join workcd g on b.workcd=g.work_code";
                    CmdBase += " where f.nobr=a.nobr and a.nobr =b.nobr";
                    ...
                    CmdBase += string.Format(@" and f.ddate='{0}'", apdate);
                    ...
                    foreach (DataRow Row1 in rq_base.Rows)
                    {
                        ...
                        aRow["到職日期"] = DateTime.Parse(Row1["indt"].ToString());
                        if (!Row1.IsNull("adate")) aRow["合同起始日"] = DateTime.Parse(Row1["adate"].ToString()).ToString("yyyy/MM/dd");
                        if (!Row1.IsNull("ddate")) aRow["合同到期日"] = DateTime.Parse(Row1["ddate"].ToString()).ToString("yyyy/MM/dd");                       
                        Dt.Rows.Add(aRow);
                    }
        ...
        public static string ConvertDataTableToHtml(DataTable dt, string nobr, string date_b, string date_e, string MonthDayB)
        {
            ...
            body += "<font color=black size=3><BR>提醒您，合同到期通知，請提前完成各項作業，謝謝</font><BR>";
            //body += "<font color=black size=2><B>Remind you that Expiration Of Contract Notice as below , Please prepare the document and notify</B></font><BR><BR>";
        ...
        public static string ConvertDataTableToHtmlDept(DataTable dt, string dept, string date_b, string date_e, string MonthDayB)
        {
            ...
            //body += "<font color=blue>*請務必在</font><table style=border:3px border='1'><td><font color=red>收到此通知後三天</font></td></table><font color=blue>內完成請假手續，三日後若未完成請假手續，系統將制本人再補請假</font><BR>";
            string body = string.Empty;
