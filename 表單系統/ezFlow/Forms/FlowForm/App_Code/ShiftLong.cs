...
public class ShiftLong : System.Web.Services.WebService
{
    ...
    public bool Run(int ID)
    {
        ...
        foreach (var rAppS in dtAppS)
        {
            ...
            if (rAppS.bSign)
            {
                JBHR.Dll.Att.ShiftRote.BaseTtsSave(rAppS.sNobr, rAppS.dDate.Value, rAppS.sRotetCode, rAppS.sHoliCode, rAppS.sOtRateCode, rAppS.sNote, rAppM.sNobr, "", true);
                rAppS.sState = "3";

                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之調班單(已核准)";
            }
            else
            {
                sSubject = "【通知】(" + rAppS.sNobr + ")" + rAppS.sName + " 之調班單(已退簽)";
            }
