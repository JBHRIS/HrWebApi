 ...
 * 檔案路徑：~\Customer\JBHR2\人事薪系統\傑報人事薪資系統\Reports\SalForm\ZZ51B_Report.cs
 * 功能用途：
 *  
 * 
 * 版本記錄：
...
{
    public partial class ZZ51B_Report : JBControls.JBForm
    {
        SalDataSet ds = new SalDataSet();
        string nobr_b, nobr_e, year, ser_nob, ser_noe, yrformat_b, yrformat_e, type_data, reporttype, username, comp_name, CompId;
        bool exportexcel;
        List<JBModule.Data.Linq.YRTAX> yrtaxlist;
        Dictionary<string, object> yrparameters;
        public ZZ51B_Report(string nobrb, string nobre, string _year, string sernob, string sernoe, string yrformatb, string yrformate, string typedata, string _reporttype, bool _exportexcel, string _username, string compname, string _CompId, List<JBModule.Data.Linq.YRTAX> YrtaxList, Dictionary<string, object> YrParameters)
        {
            InitializeComponent();
            nobr_b = nobrb; nobr_e = nobre; year = _year; ser_nob = sernob;
            ser_noe = sernoe; type_data = typedata; reporttype = _reporttype;
        ...
        private void ZZ51B_Report_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (yrparameters == null)
                {
                    ...
                    sqlCmd += "  from tbase a inner join basetts b on a.nobr = b.nobr inner join yrtax c on c.nobr = b.nobr ";
                ...
                else
                {
                    ...
                    var filterSQL = from b in db.TBASE
                    ...
                    var ResultList = (from y in yrtaxlist
                                      join f in filterList on y.NOBR equals f.TBASE.NOBR
                                      where 1 == 1
                                      //&& (!type_data2 || f.BASETTS.DI == "I" && f.TBASE.ACCOUNT_MA == "0")
                                      //&& (!type_data3 || f.BASETTS.DI == "D" && f.TBASE.ACCOUNT_MA == "0")
                                      //&& (!type_data4 || f.TBASE.COUNT_MA)
                                      && (!type_tr2 || !y.T_OK)
                                      && (!type_tr3 || y.T_OK)
                ...
                {                   
                    ...
                    {
                        RptViewer.LocalReport.ReportPath = _rptpath + "Rpt_zz51b1.rdlc";
