...
namespace JBHR.Sal
{
    public partial class FRM46I : JBControls.U_FIELD
    {
        public FRM46I()
        {
            InitializeComponent();
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            if (ctrl != null)//必要欄位檢查
            {
                MessageBox.Show("必要栏位未输入", "警告注记", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ctrl.Focus();
    ...
    public class ImportTransferToSalbasd : JBControls.ImportTransfer
    {
        ...
        public override bool ImportData(DataRow TransferRow, string RepeatSelectionString, out string ErrMsg)
        {
            ...
            try
            {
                ...
                JBModule.Data.Linq.SALBASD r = new JBModule.Data.Linq.SALBASD();
                r.NOBR = NOBR;
                r.ADATE = ADATE;
                ...
                r.MENO = MEMO;
                var sql = (from a in db.SALBASD where a.NOBR == r.NOBR && r.ADATE >= a.ADATE && r.ADATE <= a.DDATE && a.SAL_CODE == r.SAL_CODE select a).ToList();
                if (sql.Any())
                {
        ...
        bool DeleteSalbasd(JBModule.Data.Linq.SALBASD Instance, out string Msg)
        {
            ...
            {
                JBModule.Data.Linq.HrDBDataContext db = new JBModule.Data.Linq.HrDBDataContext();
                var sql = (from a in db.SALBASD where a.NOBR == Instance.NOBR && Instance.ADATE == a.ADATE && Instance.ADATE <= a.DDATE && a.SAL_CODE == Instance.SAL_CODE && a.DDATE == new DateTime(9999, 12, 31) select a).ToList();
                if (sql.Any())//有資料
                {
                    sql = (from a in db.SALBASD where a.NOBR == Instance.NOBR && Instance.ADATE >= a.ADATE && Instance.ADATE <= a.DDATE && a.SAL_CODE == Instance.SAL_CODE select a).ToList();//取得同科目所有資料
                    var other = (from a in db.SALBASD where a.NOBR == Instance.NOBR && a.SAL_CODE == Instance.SAL_CODE && a.DDATE != new DateTime(9999, 12, 31) orderby a.DDATE descending select a).ToList();//取得同科目所有数据
                    if (other.Any())
                    {//修改前一筆資料的失效日期為9999/12/31
                        var last = other.First();
                        last.DDATE = new DateTime(9999, 12, 31);
                    }
                    db.SALBASD.DeleteAllOnSubmit(sql);
                    db.SubmitChanges();
                }
            }
            catch (Exception ex)
        ...
        bool InsertSalbasd(JBModule.Data.Linq.SALBASD Instance, out string Msg)
        {
            Instance.AMT = JBModule.Data.CEncrypt.Number(Instance.AMT);
            Msg = "";
            try
            {
                JBModule.Data.Linq.HrDBDataContext db = new JBModule.Data.Linq.HrDBDataContext();
                var sql = (from a in db.SALBASD where a.NOBR == Instance.NOBR && Instance.ADATE >= a.ADATE && Instance.ADATE <= a.DDATE && a.SAL_CODE == Instance.SAL_CODE select a).ToList();
                if (sql.Any())
                {
                    if (sql.First().DDATE < new DateTime(9999, 12, 31))
                    {
        ...
        bool UpdateSalbasd(JBModule.Data.Linq.SALBASD Instance, out string Msg)
        {
            ...
            {
                JBModule.Data.Linq.HrDBDataContext db = new JBModule.Data.Linq.HrDBDataContext();
                var sql = (from a in db.SALBASD where a.NOBR == instanceRow.NOBR && instanceRow.ADATE == a.ADATE && a.SAL_CODE == instanceRow.SAL_CODE select a).ToList();
                if (sql.Any())
                {
                    if (sql.First().DDATE < new DateTime(9999, 12, 31))
                    {
        ...
        JBModule.Data.Linq.SALBASD GetSalbasd(string EmployeeId, DateTime Adate, string SalCodeDisp)
        {
            string msg = "";
            if (ColumnValidate(EmployeeId, "員工編號", TransferCheckDataField.DisplayName, out msg))
            {
                if (ColumnValidate(SalCodeDisp, "薪資代碼", TransferCheckDataField.RealCode, out msg))
                {
                    string salcode = msg;
                    JBModule.Data.Linq.HrDBDataContext db = new JBModule.Data.Linq.HrDBDataContext();
                    var sql = from a in db.SALBASD where a.NOBR == EmployeeId && Adate >= a.ADATE && Adate <= a.DDATE && a.SAL_CODE == salcode select a;
                    if (sql.Any())
                    {
        ...
        public override bool TransferToRow(DataRow SourceRow, DataRow TargetRow)
        {
            ...
            var salbasd = GetSalbasd(TargetRow["員工編號"].ToString(), Convert.ToDateTime(TargetRow["異動日期"]), TargetRow["薪資代碼"].ToString());
            if (salbasd != null)
            {
                TargetRow["異動前金額"] = salbasd.AMT;
                TargetRow["差異金額"] = Convert.ToDecimal(TargetRow["異動後金額"]) - salbasd.AMT;
                if (salbasd.DDATE != new DateTime(9999, 12, 31))
                {
                    if (TargetRow.Table != null && TargetRow.Table.Columns.Contains("錯誤註記"))
                    {
            ...
            {
                salbasd = GetSalbasd(TargetRow["員工編號"].ToString(), new DateTime(9999,12,31), TargetRow["薪資代碼"].ToString());
                if (salbasd != null)//在異動日期之後還有資料
                {
                    TargetRow["錯誤註記"] = "已存在更新的調薪資料";
                    TargetRow["異動前金額"] = salbasd.AMT;
                    TargetRow["差異金額"] = Convert.ToDecimal(TargetRow["異動後金額"]) - salbasd.AMT;                  
