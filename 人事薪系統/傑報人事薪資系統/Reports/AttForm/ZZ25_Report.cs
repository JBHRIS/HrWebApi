    ...
    public partial class ZZ25_Report : JBControls.JBForm
    {
        ...
        private void ZZ25_Report_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                sqlCmd += ",c.d_no_disp as dept,c.d_name,c.d_ename,b.indt,b.cindt,b.di";
                ...
                sqlCmd1 += " from abs a";
                sqlCmd1 += " left outer join hcode b on a.h_code=b.h_code";              
                ...
                if (date_type == "1")
                {
                    string sqlCmd2 = "select a.nobr,a.bdate,a.edate,b.h_code_disp as h_code,b.h_name,a.tol_hours";                  
                    sqlCmd2 += " from abs a";                  
                    sqlCmd2 += " left outer join hcode b on a.h_code=b.h_code";
                ...
                {                               
                    ...
                    if (row != null)
                    {
                        ...
                        else if (Row["year_rest"].ToString() == "2")
                        {
                            ...
                            if (Row["unit"].ToString().Trim() == "天")
                                Row["tol_hours"] = decimal.Parse(Row["tol_hours"].ToString());
        ...
        void Export(DataTable DT)
        {
            ...
            if (reporttype == "0")
            {
                ...
                foreach (DataRow Row in DTrow)
                {
                    ...
                    aRow["集團到職日"] = DateTime.Parse(Row["cindt"].ToString());
                    ...
                    if (Row["tol_hours1"].ToString() == "")
                        ...
                        aRow["得假天數"] = decimal.Parse(Row["tol_hours1"].ToString());
                    aRow["請假代碼"] = Row["h_code2"].ToString();
                    aRow["請假名稱"] = Row["h_name2"].ToString();
                    if (Row["tol_hours2"].ToString() == "")
                        aRow["請假天數"] = 0;
                    else
                        aRow["請假天數"] = decimal.Parse(Row["tol_hours2"].ToString());
