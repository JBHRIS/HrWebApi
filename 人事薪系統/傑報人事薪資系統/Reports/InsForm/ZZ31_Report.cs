 ...
 * 功能名稱：勞健保保險資料
 * 功能代號：ZZ31
 * 功能路徑：報表列印 > 保險 > 勞健保保險資料
 * 檔案路徑：~\Customer\JBHR2\人事薪系統\傑報人事薪資系統\Reports\InsForm\ZZ31_Report.cs
 * 功能用途：
 *  用於產出勞健保保險資料
 */
/* 版本記錄：
 ...
 *    日期        人員              版本          單號                                  說明
 * ------------------------------------------------------------------------------------------------------
 * 2022/02/21   Daniel Chih     Ver 1.0.01  ITCT-F01-220088-寶齡富錦-20220221   1. 修正選擇【全部】時，起迄區間無作用的問題
 * 
 */
/* ======================================================================================================
 */
/* 【測試資料 - BPBFHR】：
 *  保險狀況：全部
 *  員工編號：P845031 ~ P845031
 *  編制部門：20106 ~ ZZZZ
 *  起迄日期：2019/01/01 ~ 2022/02/28
 *  投保單位：411 ~ 411
 *  資料內容：全部
 *  報表種類：健保資料(投保金額)
 *  勾選項目：匯出Excel
            ...
            try
            {
                ...
                { 
                    ...
                    default:
                        ...
                        sqlCmd1 += string.Format(@" and b.s_no_disp between '{0}' and '{1}'", sno_b, sno_e);
                        sqlCmd1 += string.Format(@" and in_date <= '{0}'", date_e);
                        sqlCmd1 += string.Format(@" and out_date >= '{0}'", date_b);
                        sqlCmd1 += " order by a.nobr";
                        break;
                }
