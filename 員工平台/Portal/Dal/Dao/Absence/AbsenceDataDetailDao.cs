...
{
    public class AbsenceDataDetailDao : BaseWebAPI<AbsenceDataDetailApiRow>
    {

        public AbsenceDataDetailDao() : base()
        {
                    ...
                    if (Vdb.Payload != null && Vdb.Data != null)
                    {
                        ...
                        if (oSource != null)
                        {
                            ...
                            if (oSource.state)
                            {
                                ...
                                var rsTarget = new List<AbsenceDataDetailRow>();
                                foreach (var rSource in rsSource)
                                {
                                    var rTarget = new AbsenceDataDetailRow();
                                    rTarget.EmpId = rSource.Nobr;
                                    
                                    
                                    rTarget.Hcode = rSource.HCode;
                                    rTarget.Use = rSource.TotHours;
                                    rTarget.UnitCode = rSource.Unit;
                                    foreach (var rrSource in rSource.CalAbsHoursDetails)
                                    {
                                        rTarget.DateB = rSource.CalAbsHoursDetails[0].StartDateTime;
                                        rTarget.TimeB = rSource.CalAbsHoursDetails[0].StartDateTime.ToString("HHmm");
                                        rTarget.DateE = rrSource.EndDateTime;
                                        rTarget.TimeE = rrSource.EndDateTime.ToString("HHmm");
                                    }
