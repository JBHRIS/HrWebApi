...
namespace JBHR.TRA
{
	public partial class FRM91 : JBControls.JBForm
	{
        CheckControl cc;
		public FRM91()
		{
		...
		{
            // TODO:  這行程式碼會將資料載入 'traDS1.TRTYPE' 資料表。您可以視需要進行移動或移除。
            this.tRTYPETableAdapter.Fill(this.traDS1.TRTYPE);
            fullDataCtrl1.DataAdapter = tRTYPETableAdapter;
            ...
            fullDataCtrl1.CodeColumn = "TRTYPE.TR_TYPE";//**代碼權限設定**
            fullDataCtrl1.CodeSource = "TRTYPE";//**代碼權限設定**
		...
		private void fullDataCtrl1_BeforeSave(object sender, JBControls.FullDataCtrl.BeforeEventArgs e)
		{
            ...
            {
                if (GetTrType(txtTR_TYPE_DISP.Text) != null)
                {
                    MessageBox.Show("「課程類別代碼」已使用，請重新輸入。", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtTR_TYPE_DISP.Focus();
                    e.Cancel = true;
                    return;
                }
            }
			if (!e.Cancel)
			{
                ...
                {
                    e.Values["TR_TYPE"] = Guid.NewGuid().ToString();
                }
				e.Values["key_man"] = MainForm.USER_NAME;
				e.Values["key_date"] = DateTime.Now;
			}
		}
        JBModule.Data.Linq.TRTYPE GetTrType(string TrTypeDisp)
        {
            JBModule.Data.Linq.HrDBDataContext db = new JBModule.Data.Linq.HrDBDataContext();
            var sql = from a in db.TRTYPE
                      where a.TR_TYPE_DISP == TrTypeDisp
                      select a;
