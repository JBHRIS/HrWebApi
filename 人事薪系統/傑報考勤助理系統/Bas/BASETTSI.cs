...
namespace JBHR.Bas
{
    public partial class BASETTSI : JBControls.JBForm
    {
        ...
        private void comboBoxSheet_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            cbxName.Items.Clear();
            cbxRotet.Items.Clear();
            cbxHoliCD.Items.Clear();
            foreach (DataColumn col in TtsData.Columns)
            {
                ...
                cbxRotet.Items.Add(col.ColumnName);
                cbxHoliCD.Items.Add(col.ColumnName);
            }
        ...
        void CombBoxSettingDefault()
        {
            ...
            if (cbxRotet.Items.Contains(lblTtscd.Text)) cbxRotet.Text = lblRotet.Text;

            if (cbxHoliCD.Items.Contains(lblTtscd.Text)) cbxHoliCD.Text = lblHoliCD.Text;
        ...
        void Run(bool isOK)
        {
            ...
            var dtTtscd = (from a in db.TTSCD
                           ...
                           select a).ToList();
            var dtRotet = (from a in db.ROTET
                           where
                               db.GetCodeFilter("ROTET", a.ROTET1, MainForm.USER_ID, MainForm.COMPANY, MainForm.ADMIN).Value
                           select a).ToList();
            var dtHoliCD = (from a in db.HOLICD
                           where
                               db.GetCodeFilter("HOLICD", a.HOLI_CODE, MainForm.USER_ID, MainForm.COMPANY, MainForm.ADMIN).Value
                           select a).ToList();
                ...
                if (TtsData != null)
                    foreach (DataRow row in TtsData.Rows)
                    {
                        ...
                        if (dtBASETTS.Rows.Count > 0)
                        {
                            ...
                            if (cbxRotet.Text.Trim().Length > 0)
                            {
                                if (TtsData.Columns.Contains(cbxRotet.Text))
                                {
                                    data["rotet"] = row[cbxRotet.Text].ToString().Trim();
                                    if (!dtRotet.Where(p => p.ROTET_DISP.Trim() == data["rotet"].ToString().Trim()).Any())//
                                    {
                                        error++;
                                        if (stateList.ContainsKey(rowCount)) stateList[rowCount] = JBModule.Data.CNPOI.ExcelRowState.DataCheck;
                                        else
                                    ...
                                    else data["rotet"] = dtRotet.Where(p => p.ROTET_DISP.Trim() == data["rotet"].ToString().Trim()).First().ROTET1;
                                }
                                else
                                {
                                    mapping++;
                            ...
                            if (cbxHoliCD.Text.Trim().Length > 0)
                            {
                                if (TtsData.Columns.Contains(cbxHoliCD.Text))
                                {
                                    data["holi_code"] = row[cbxHoliCD.Text].ToString().Trim();
                                    if (!dtHoliCD.Where(p => p.HOLI_CODE_DISP.Trim() == data["holi_code"].ToString().Trim()).Any())//
                                    {
                                        error++;
                                        if (stateList.ContainsKey(rowCount)) stateList[rowCount] = JBModule.Data.CNPOI.ExcelRowState.DataCheck;
                                        else
                                    ...
                                    else data["holi_code"] = dtHoliCD.Where(p => p.HOLI_CODE_DISP.Trim() == data["holi_code"].ToString().Trim()).First().HOLI_CODE;
                                }
                                else
                                {
                                    mapping++;
