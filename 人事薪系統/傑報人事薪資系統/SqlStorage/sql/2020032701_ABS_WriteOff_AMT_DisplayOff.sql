Update jqUserColumn Set Display = 0 ,DataType = 'nvarchar'
WHERE ColumnName = '扣款金額' and TableName = 'View_ABSWriteOff'
GO

Update jqColumn Set Display = 0 ,DataType = 'nvarchar'
WHERE ColumnName = '扣款金額' and TableName = 'View_ABSWriteOff'
GO
...
/****** Object:  View [dbo].[VIEW_ABSWRITEOFF]    Script Date: 2020/3/27 上午 11:13:26 ******/
SET ANSI_NULLS ON
GO
...
SELECT  TOP (100) PERCENT ABS.屬性, ABS.NOBR AS 員工編號, dbo.BASE.NAME_C AS 員工姓名, ABS.YYMM AS 計薪年月, dbo.ROTE.ROTE_DISP AS 班別, 
               ...
               ABS.KEY_MAN AS 登錄者, ABS.KEY_DATE AS 登錄日, CAST(CASE WHEN SUM(SALABS.扣款金額) > 0 THEN '有' ELSE NULL END AS nvarchar(50)) 
               AS 扣款金額
