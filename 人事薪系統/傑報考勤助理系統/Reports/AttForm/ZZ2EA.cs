...
{
    public partial class ZZ2EA : JBControls.JBForm
    {
        ZZ2EA_Report zz2ea_report;
        public ZZ2EA()
        {
            InitializeComponent();
        }

        private void ZZ2EA_Load(object sender, EventArgs e)
        {
        ...
        private void Create_Report_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DateTime.TryParseExact(date_t.Text, "yyyy/MM/dd", null, System.Globalization.DateTimeStyles.AllowWhiteSpaces, out dt) == false)
                {
                    date_t.Focus();
                    return;
                }
                if (zz2ea_report != null)
                {
                    zz2ea_report.Dispose();
                    zz2ea_report.Close();
                }
                ...
                zz2ea_report = new ZZ2EA_Report(nobrb, nobre, deptb, depte, workb, worke, compb, compe, dateb, datee, datet, inout, _exportexcel, reporttype, _w8, data_report,MainForm.COMPANY_NAME);
                zz2ea_report.Show();
            }
            catch (Exception Ex)
            {
