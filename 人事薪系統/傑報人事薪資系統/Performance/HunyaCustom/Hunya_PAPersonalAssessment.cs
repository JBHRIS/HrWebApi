...
{
    public partial class Hunya_PAPersonalAssessment : JBControls.JBForm
    {
        public Hunya_PAPersonalAssessment()
        {
            InitializeComponent();
        }

        private void Hunya_PAPersonalAssessment_Load(object sender, EventArgs e)
        {
            // TODO: 這行程式碼會將資料載入 'hunya_Performance.V_BASE' 資料表。您可以視需要進行移動或移除。
            this.v_BASETableAdapter.Fill(this.hunya_Performance.V_BASE);
            this.hunya_PALevelCodeTableAdapter.Fill(this.hunya_Performance.Hunya_PALevelCode, MainForm.USER_ID, MainForm.COMPANY, MainForm.ADMIN);
        }

        private void JQPAPersonalAssessment_RowInsert(object sender, JBControls.JBQuery.RowInsertEventArgs e)
        {
            Hunya_PAPersonalAssessment_ADD frm = new Hunya_PAPersonalAssessment_ADD();
            frm.ShowDialog();
        }

        private void JQPAPersonalAssessment_RowUpdate(object sender, JBControls.JBQuery.RowUpdateEventArgs e)
        {
            Hunya_PAPersonalAssessment_ADD frm = new Hunya_PAPersonalAssessment_ADD();
            Dictionary<string, object> values = JQPAPersonalAssessment.SelectedKey as Dictionary<string, object>;
        ...
        private void JQPAPersonalAssessment_RowDelete(object sender, JBControls.JBQuery.RowDeleteEventArgs e)
        {
            ...
            var instance = db.Hunya_PAPersonalAssessment.SingleOrDefault(p => p.AK == Convert.ToInt32(Autokey));
            if (instance != null)
            {
                ...
                db.Hunya_PAPersonalAssessment.DeleteOnSubmit(instance);
                db.SubmitChanges(); 
            }
        }
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            frm.FieldForm = new Hunya_PAPersonalAssessment_Import();
            Hunya_PAPersonalAssessment_Import.Hunya_PAPersonalAssessment_ImportData HPBG_ImportData = new Hunya_PAPersonalAssessment_Import.Hunya_PAPersonalAssessment_ImportData();
            ...
            frm.DataTransfer.CheckData.Add("考核等級", this.hunya_Performance.Hunya_PALevelCode.Select(p => new JBControls.CheckImportData { DisplayCode = p.PALevelCode_DISP, RealCode = p.PALevelCode, DisplayName = p.PALevelCode_Name }).ToList());
            ...
            frm.DataTransfer.ColumnList.Add("考核年月", typeof(string));
