...
namespace OldDal.Dao.Att
{
    public class AttcardDao
    {
        ...
        public List<AttcardRow> GetAttcard(string sNobr, DateTime dDate)
        {
            ...
            foreach (var rVdb in Vdb)
            {
                if (rVdb.OnCardTime48.Length > 0)
                    rVdb.DateTimeB = rVdb.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rVdb.OnCardTime48));

                if (rVdb.OffCardTime48.Length > 0)
                    rVdb.DateTimeE = rVdb.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rVdb.OffCardTime48));
            }
        ...
        public bool IsCardTime(string sNobr, DateTime dDate, string sTimeB, string sTimeE)
        {
            ...
            dDateTimeB = dDate.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeB));    //加班開始日期時間
            dDateTimeE = dDate.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeE));    //加班結束日期時間
        ...
        public bool IsCardTime(string sNobr, DateTime dDateB , DateTime dDateE, string sTimeB, string sTimeE)
        {
            bool bCardTime = false;
            DateTime dDateTimeB, dDateTimeE;
            dDateTimeB = dDateB.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeB));    //加班開始日期時間
            dDateTimeE = dDateE.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(sTimeE));    //加班結束日期時間
