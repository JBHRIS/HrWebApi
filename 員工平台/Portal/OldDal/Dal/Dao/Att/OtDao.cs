...
namespace OldDal.Dao.Att
{
    public class OtDao
    {
        ...
        public int DeleteOt(string sNobr, DateTime dDateB, string sTimeB, DateTime dDateE, string sTimeE, string sType)
        {
            ...
            foreach (var rOT in rsOT)
            {
                dDateTimeB = rOT.BDATE.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rOT.BTIME.Trim()));
                dDateTimeE = rOT.BDATE.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rOT.ETIME.Trim()));
        ...
        /// <param name="InterVal">加班時數單位</param>
        /// <returns>decimal</returns>
        public decimal GetCalculate(string sNobr, string sOtCat, DateTime dDateB, DateTime dDateE, string sTimeB, string sTimeE, string sOtrcd, decimal iException = 0, string sRoteCode = "", bool bCalculateRes = true, bool bTime24 = false, Decimal InterVal = 0.5M)
        {
            if (bTime24)
            ...
            //套用加班計算公式
            OldBll.Att.Ot oOt = new OldBll.Att.Ot();
            ...
            bool bException = iException > 0;
            {
                ...
                if (rAttend != null)
                {
                    ...
                    if (rRote != null)
                    {
                        DateTime DateTimeB = dDateB.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rRote.OnTime));
                        DateTime DateTimeE = dDateB.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rRote.OffTime));
                        ...
                        if (bCalculateRes)
                        {
                            ...
                            if (!lsRoteHoliDayRoteCode.Contains(RealRoteCode))
                            {
                                ...
                                {
                                    DateTime DateTimeResB = dDateB.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rDayResRes.ResTimeB));
                                    DateTime DateTimeResE = dDateB.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rDayResRes.ResTimeE));
                        ...
                        Vdb = oOt.GetCalculate(sTimeB, sTimeE, rsRes, 0, InterVal, (ElasticityMin));
                    }
                }
            }
        ...
        public int OtSave(string sNobr, string sOtCat, DateTime dDateB, DateTime dDateE, string sTimeB, string sTimeE, decimal iHour, string sOtrcd, string sRoteCode, string sDeptsCode = "", string sAbstCode = "", string sNote = "", string sKeyMan = "System", string sSerno = "", bool bTime24 = false, DateTime? dDateA = null, DateTime? dDateD = null)
        {
            ...
            var rsGetSalaryYymm = oSalaryLockDao.GetSalaryYymm(lsSalaryDateBE, 25);
            ...
            if (!rsOt.Where(p => p.Date == dDateB.Date && p.TimeB == sTimeB).Any())
                if (!rsOt.Where(p => p.DateTimeB < DateTimeE && p.DateTimeE > DateTimeB).Any())
                {
                    ...
                    var rOT = new OT();
                    OldBll.Tools.DefaultData.SetRowDefaultValue(rOT);
                    rOT.NOBR = sNobr;
                    rOT.BDATE = dDateB;
                    rOT.BTIME = sTimeB;
                    rOT.ETIME = sTimeE;
        ...
        /// <param name="bTime24">24小時計算</param>
        /// <returns>int</returns>
        public int Ot1Save(string sNobr, string sOtCat, DateTime dDateB, DateTime dDateE, string sTimeB, string sTimeE, decimal iHour, string sOtrcd, string sRoteCode, string sDeptsCode = "", string sAbstCode = "", string sNote = "", string sKeyMan = "System", string sSerno = "", bool bTime24 = false)
        {
            if (bTime24)
                ConvertTime24To48(sNobr, ref dDateB, ref dDateE, ref sTimeB, ref sTimeE);
            ...
            if (!rsOt.Where(p => p.Date == dDateB.Date && p.TimeB == sTimeB).Any())
                if (!rsOt.Where(p => p.DateTimeB < DateTimeE && p.DateTimeE > DateTimeB).Any())
                {
                    ...
                    var rOT1 = new OT1();
                    OldBll.Tools.DefaultData.SetRowDefaultValue(rOT1);
                    rOT1.NOBR = sNobr;
                    rOT1.BDATE = dDateB;
                    rOT1.BTIME = sTimeB;
                    rOT1.ETIME = sTimeE;
                    rOT1.TOT_HOURS = iHour;
                    rOT1.OT_HRS = (sOtCat == "1") ? rOT1.TOT_HOURS : 0;
                    rOT1.REST_HRS = (sOtCat == "2") ? rOT1.TOT_HOURS : 0;
                    rOT1.OT_DEPT = sDeptsCode;
                    rOT1.KEY_MAN = sKeyMan;
                    rOT1.NOTE = sNote.Length > 100 ? sNote.Substring(0, 99) : sNote;// +";" + oOtDetail.sRote;
                    rOT1.YYMM = sYYMM;
                    rOT1.OTRCD = sOtrcd;
                    rOT1.OT_EDATE = dDateD;
                    rOT1.OT_ROTE = sRoteCode;
                    rOT1.SERNO = sSerno;
                    dcHr.OT1.InsertOnSubmit(rOT1);
        ...
        public List<OtRow> GetOt(List<string> lsNobr, DateTime dDateB, DateTime dDateE)
        {
            ...
            foreach (var rVdb in Vdb)
            {
                if (rVdb.TimeB.Length > 0)
                    rVdb.DateTimeB = rVdb.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rVdb.TimeB));

                if (rVdb.TimeE.Length > 0)
                    rVdb.DateTimeE = rVdb.Date.AddMinutes(OldBll.Tools.TimeTrans.ConvertHhMmToMinutes(rVdb.TimeE));
            }
        ...
        /// <param name="dDateE">結束日期</param>
        /// <returns>List OtRow</returns>
        public List<OtRow> GetOt1(string sNobr, DateTime dDateB, DateTime dDateE)
        {
            var Vdb = (from c in dcHr.OT1
                       join b in dcHr.BASE on c.NOBR equals b.NOBR
                       //join o in dcHr.OTRCD on c.OTRCD equals o.OTRCD1 
                       where c.NOBR == sNobr
                       && dDateB.Date <= c.BDATE.Date
