...
namespace ezEngineServices.core
{
    public class COrg
    {
        ...
        public COrg(ezFlowContext dcFlow)
        {
            this.dcFlow = dcFlow;
        }
        ...
        public CMan GetAgent(int idProcess, string Role_idSource, string Emp_idSource)
        {
            ...
            var rProcessFlow = (from c in dcFlow.ProcessFlows
                                join r in dcFlow.Roles on new { Role = c.Role_id, Emp = c.Emp_id } equals new { Role = r.id, Emp = r.Emp_id }
                                join d in dcFlow.Depts on r.Dept_id equals d.id
            ...
            if (rProcessFlow != null)
            {
                ...
                if (FlowTree_id.Length > 0)
                {
                    ...
                    //可代理的單據
                    var rsCheckAgentFlowTree = (from c in dcFlow.CheckAgentFlowTrees
                                                where (from d in rsCheckAgentSql where d.Guid == c.CheckAgent_Guid select 1).Any()
                                                select c).ToList();

                    //可簽核的部門
                    var rsCheckAgentDept = (from c in dcFlow.CheckAgentDepts
                                            join e in dcFlow.Depts on c.Dept_id equals e.id
                                            where (from d in rsCheckAgentSql where d.Guid == c.CheckAgent_Guid select 1).Any()
        ...
        public CMan GetManager(int idProcess, string Role_idMinion, bool EmpSameUp = true)
        {
            ...
            {
                //先採用角色取得上層主管
                var rRole = (from r in dcFlow.Roles
                ...
                if (rRole != null)
                {
                    ...
                    //尋找上層角色的工號
                    var rRoleParent = (from r in dcFlow.Roles
                                       where r.id == Role_idMinion
                                       && dDate.Date >= r.dateB
                                       && dDate.Date <= r.dateE
                    ...
                    else  //有可能上層角色是空白的 所以必須要用部門來判斷
                    {
                        ...
                        do
                        {
                            ...
                            //用找到的部門到角色裡尋找主管
                            var rRoleManage = (from r in dcFlow.Roles
        ...
        {
            string PathOld = (from r in dcFlow.Roles
                              join d in dcFlow.Depts on r.Dept_id equals d.id
                              where r.id == OldRole
                              select d.path).FirstOrDefault();

            string PathNew = (from r in dcFlow.Roles
                              join d in dcFlow.Depts on r.Dept_id equals d.id
                              where r.id == NewRole
                              select d.path).FirstOrDefault();
