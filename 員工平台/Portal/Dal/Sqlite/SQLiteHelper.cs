...
        public SQLiteHelper()
        {
          //  string _dbPath = Path.Combine( System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal), "\\Data\\slMain.db");

            string _dbPath = string.Format("{0}\\Data\\{1}", System.Web.HttpContext.Current.Server.MapPath("~/"), "slMain.db"); //取得DB

            _Connstring = string.Format("Data Source={0};", _dbPath);
        }
