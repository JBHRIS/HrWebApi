...
namespace Dal.Dao.Share
{
    public class ShareGetQuestionMainDao : BaseWebAPI<ShareGetQuestionMainApiRow>
    {
        ...
        public async Task<APIResult> PostAsync(ShareGetQuestionMainConditions Cond, CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        public APIResult Post(ShareGetQuestionMainConditions Cond, CancellationToken cancellationToken = default(CancellationToken))
        {
            AuthenticationHeaderBearerTokenValue = Cond.AccessToken;
        ...
        public async Task<APIResult> GetDataAsync(ShareGetQuestionMainConditions Cond, CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        public APIResult GetData(ShareGetQuestionMainConditions Cond, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (Vdb.Status)
            {
                if (Vdb.Data != null)
                {
                    ...
                    {
                        //實作DTO轉換
                        var oSource = Vdb.Data as ShareGetQuestionMainApiRow;
                        if (oSource != null)
                        {
                            ...
                            if (oSource.state)
                            {
                                if (oSource != null)
                                {
                                    ...
                                    var rsTarget = new List<ShareGetQuestionMainRow>();

                                    //把api的Data轉成我們的Data
                                    foreach (var rSource in rsSource)
                                    {
                                        var rTarget = new ShareGetQuestionMainRow();
