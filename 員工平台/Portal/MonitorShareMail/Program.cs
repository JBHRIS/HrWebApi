...
namespace MonitorShareMail
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var dcMain = new dcMainDataContext();
            ...
            //寄信
            {
                ...
                if (_MonitorSendMail)
                {
                    ...
                    foreach (var rSendQueue in rsSendQueue)
                    {
                        ...
                        var Sucess = rSendQueue.Sucess;
                        var InsertMan = rSendQueue.InsertMan;
                        ...
                        //if (!Sucess)
                        {
                            ...
                            var rMAILQUEUE = new MAILQUEUE();
                            rMAILQUEUE.GUID = Guid.NewGuid().ToString();
                            rMAILQUEUE.FROM_ADDR = "emchr@mail.emctw.com";//emchr@mail.emctw.com
                            rMAILQUEUE.FROM_NAME = "Performance";
                            rMAILQUEUE.TO_ADDR = ToAddr;
                            rMAILQUEUE.TO_NAME = ToName;
                            rMAILQUEUE.SUBJECT = Subject;
                            rMAILQUEUE.BODY = Body;
                            rMAILQUEUE.RETRY = 0;
                            rMAILQUEUE.SUCCESS = false;
                            rMAILQUEUE.SUSPEND = false;
                            rMAILQUEUE.FREEZE_TIME = DateTime.Now;
                            rMAILQUEUE.KEY_MAN = InsertMan;
                            rMAILQUEUE.KEY_DATE = DateTime.Now;
                            dcMain.MAILQUEUE.InsertOnSubmit(rMAILQUEUE);

                            var oMessageLog = oMain.MessageLog("0", "SendMail", JsonConvert.SerializeObject(rSendQueue), "MonitorShare", "", InsertMan) ;
