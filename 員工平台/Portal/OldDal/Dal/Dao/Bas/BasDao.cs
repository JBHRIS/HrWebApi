    ...
    public class BasDao
    {
        ...
        public List<BaseTable> GetBase(string sNobr = "", bool bIn = true)
        {
            var Vdb = (from b in dcHr.BASE
                       ...
                       on b.NOBR equals b1.NOBR
                       where (bIn ? arrTtscode.Contains(b1.TTSCODE) : true)
                       && (sNobr == "" || b1.NOBR == sNobr)
        ...
        {
            var Vdb = (from b in dcHr.BASE
                       where lsNobr.Contains(b.NOBR)
                       select new BaseTable
                       {
                           Nobr = b.NOBR.Trim(),
                           NameC = b.NAME_C.Trim(),
                           NameE = b.NAME_E.Trim(),
                           Name = b.NOBR.Trim() + "," + b.NAME_C.Trim(),
                       }).ToList();
        ...
        public List<BaseTable> GetBaseByNobr(string sNobr, string sDeptCate = "1", bool bDeptDown = true, bool bIn = true)
        {
            ...
            var Vdb = (from b in dcHr.BASE
                       ...
                       && b1.ADATE.Date.CompareTo(DateNow) <= 0
                       && DateNow.CompareTo(b1.DDATE.Value.Date) <= 0
                       && lsDept.Contains((sDeptCate == "1") ? b1.DEPT : b1.DEPTM)
                       select new BaseTable
                       {
                           Nobr = b.NOBR.Trim(),
                           NameC = b.NAME_C.Trim(),
        ...
        public List<BaseDetailTable> GetBaseByNobr(string sNobr, DateTime dDate)
        {
            var Vdb = (from b1 in dcHr.BASETTS
                       join b in dcHr.BASE
                       on b1.NOBR equals b.NOBR
                       join d in dcHr.DEPT
                       on b1.DEPT equals d.D_NO
                       join ds in dcHr.DEPTS
                       on b1.DEPTS equals ds.D_NO
                       join dm in dcHr.DEPTA
                       on b1.DEPTM equals dm.D_NO
                       join j in dcHr.JOB
                       on b1.JOB equals j.JOB1
                       join jl in dcHr.JOBL
                       on b1.JOBL equals jl.JOBL1
                       join wc in dcHr.WORKCD
                       on b1.WORKCD equals wc.WORK_CODE
                       where b1.NOBR == sNobr
                       ...
                       select new BaseDetailTable
                       {
                           ...
                           Sex = b.SEX.Trim() == "M" ? OldBll.MT.mtEnum.SexCategroy.Male : OldBll.MT.mtEnum.SexCategroy.Female,
                           IDNo = b.IDNO.Trim(),
        ...
        public List<WorkRow> GetWorkcd(string sCode = "")
        {
            var Vdb = (from c in dcHr.WORKCD
                       where (sCode == "" || c.WORK_CODE == sCode || c.WORK_ADDR == sCode)
                       select new WorkRow
                       {
        ...
        public List<BaseTable> GetAllEmp()
        {
            var Vdb = (from b1 in dcHr.BASETTS
                       ...
                       where arrTtscode.Contains(b1.TTSCODE)
                         && b1.ADATE.Date.CompareTo(DateNow) <= 0
                         && DateNow.CompareTo(b1.DDATE.Value.Date) <= 0
                       select new BaseTable
                       {
