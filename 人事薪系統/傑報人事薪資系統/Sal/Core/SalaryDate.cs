...
namespace JBHR.Sal.Core
{
    public class SalaryDate
    {
        ...
        public SalaryDate(DateTime date, bool Salary)
        {
            if (Salary)
            {
                if (date.Day > SalEndDay)
        ...
        private void AttendYYMM(DateTime date)
        {
            int EndDay = AttEndDay;
            if (AttEndDay == 0)
            {
                date = date.AddMonths(1);
                EndDay = 31;
        ...
        public static string GetSaladr(string Nobr, DateTime date)
        {
            ...
            var sql = from a in db.BASETTS where date >= a.ADATE && date <= a.DDATE.Value && a.NOBR == Nobr select a;
            if (sql.Any())
        ...
        public DateTime LastDayOfAttend
        {
            get
            {
                ...
                DateTime d2 = new DateTime(date.Year, date.Month, EndDay > days ? days : EndDay);
                return d2;
            }
        }
