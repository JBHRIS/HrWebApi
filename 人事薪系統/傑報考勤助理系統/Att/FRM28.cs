...
namespace JBHR.Att
{
    ...
    public partial class FRM28 : JBControls.JBForm
    {
        ...
        private void FRM28_Load(object sender, EventArgs e)
        {
            ...
            SystemFunction.SetComboBoxItems(ptxHcode, CodeFunction.GetHcode(false), true);
        ...
        void TimeSet()
        {
            try
            {
                ...
                var sql = from rowAtt in db.ATTEND join rowRote in db.ROTE on rowAtt.ROTE equals rowRote.ROTE1 where rowAtt.NOBR == nobr && rowAtt.ADATE.Date <= Bdate.Date && rowAtt.ROTE != "00" orderby rowAtt.ADATE descending select rowRote;
                if (sql.Any())
                {
                    var row = sql.First();
        ...
        public static bool AutoABSD(string Nobr, string Hcode, DateTime dDate, string guid, decimal AbsHours, bool OverWrite = false)
        {
            var myDB = new JBModule.Data.Linq.HrDBDataContext();
        ...
        private void buttonABSD_Click(object sender, EventArgs e)
        {
            if (fullDataCtrl1.EditType == JBControls.FullDataCtrl.EEditType.None)
            {
                ...
                {
                    var sql = from a in HcodeList where a.H_CODE == ptxHcode.SelectedValue.ToString() select a;
                    if (sql.Any())
                    {
                        var myDB = new JBModule.Data.Linq.HrDBDataContext();
        ...
        private void buttonCheckABSD_Click(object sender, EventArgs e)
        {
            ...
            foreach (var it in dsAtt.ABS)
            {
                if (checkBoxForce.Checked)
                    AutoABSDWithForce(it.NOBR, it.H_CODE, it.BDATE, it.Guid, it.TOL_HOURS);
                else
                    AutoABSD(it.NOBR, it.H_CODE, it.BDATE, it.Guid, it.TOL_HOURS, checkBoxABSD.Checked);
            }
