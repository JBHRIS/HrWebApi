...
namespace JBHR.Dll
{
    ...
    public static class Tools
    {
        ...
        /// <param name="sDeptm">部門</param>
        /// <returns>string</returns>
        public static string GetDeptmManageToLevel(string sNobr, string sDeptm, string sLevel)
        {
            string[] arr = { "1", "4", "6" };
            ...
            if (sDeptm == string.Empty)
            {
                ...
                //先抓部門裡設定的主管不是空白就直接使用
                var rDeptm = Deptm(sDeptm).FirstOrDefault();
                if (rDeptm != null && rDeptm.sNobr.Trim().Length > 0 && rDeptm.sNobr.Trim() != sNobr && rDeptm.sDeptTree == sLevel)
                    return rDeptm.sNobr.Trim();
            }
            ...
            do
            {
                var rsBases = Deptm(sDeptm).FirstOrDefault();
                if (rsBases.sNobr == sNobr)
                {
                    var rDeptm = Deptm().Where(p => p.sDeptCode == sDeptm).FirstOrDefault();
                    if (rDeptm == null || rDeptm.sDeptParent.Trim() == string.Empty)
                        break;
