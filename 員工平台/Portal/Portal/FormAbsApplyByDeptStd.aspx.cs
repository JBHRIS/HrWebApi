...
namespace Portal
{
    public partial class FormAbsApplyByDeptStd : WebPageBase
    {
        ...
        private void SetDept(string sNobr)
        {
            ...
            if (Result.Status)
            {
                if (Result.Data != null)
                {
                    ...
                    if (rs.Count != 0)
                    {
                        if (ddlDept.Items.FindItemByValue(rs[0].DeptCode) != null)
                            ddlDept.Items.FindItemByValue(rs[0].DeptCode).Selected = true;
                    }
                }
            }
        }
        ...
        protected void ddlDept_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            ...
            Dept.Add(ddlDept.SelectedValue);

            var rsPeopleByDept = new List<PeopleByDeptRow>();
            {
                var oAllPeopleByDept = new AllPeopleByDeptDao();
                ...
                var Result = oAllPeopleByDept.GetData(PeopleByDeptCond);
            ...
            if(rsPeopleByDept.Count!=0)
            {
                ...
                //txtNameAppS.Text = rs.FirstOrDefault().Text;
                SetName(rs.FirstOrDefault().Value);
                SetName1(rs.FirstOrDefault().Value);
            }
            else
            {
                txtNameAppS.Items.Clear();
                txtNameAppS.Text = "";

                txtNameAgent1.Items.Clear();
                txtNameAgent1.Text = "";
