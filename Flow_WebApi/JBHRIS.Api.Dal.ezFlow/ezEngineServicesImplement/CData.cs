...
namespace JBHRIS.Api.Dal.ezFlow.ezEngineServicesImplement
{
    ...
    public class CData : ICData_Dal
    {
        ...
        public CData(ezFlowContext context)
        {
        ...
        public List<CheckAgentDataRow> GetCheckAgentData(string EmpId)
        {
            ...
            var rsCheckAgent = (from ca in this._context.CheckAgents
                                where ca.Emp_idSource == EmpId
                                select ca).ToList();

            var lsGuid = rsCheckAgent.Select(p => p.Guid).ToList();

            var rsForm = (from c in this._context.CheckAgentFlowTrees
                          join d in this._context.wfForms on c.FlowTree_id equals d.sFlowTree
            ...
            var rsRole = (from c in this._context.Roles
            ...
            var rsDept = (from c in this._context.Depts
                          where lsDept.Contains(c.id)
                          select c).ToList();

            var rsPos = (from c in this._context.Pos
                         where lsPos.Contains(c.id)
                         select c).ToList();

            var rsEmp = (from c in this._context.Emps
        ...
        {
            var Vdb = (from c in this._context.CheckAgentFlowTrees
                       join f in this._context.FlowTrees on c.FlowTree_id equals f.id
        ...
        {
            var Vdb = (from c in this._context.EmpAgentDates
        ...
        public List<FlowNodeApNameRow> GetFlowNodeAppName(List<string> lsFlowNodeId)
        {
            ...
            var rsFlowNode = (from c in this._context.FlowNodes
            ...
            foreach (var rFlowNode in rsFlowNode)
            {
                ...
                {
                    case "3": //主管審核
                        var rNodeMang = (from c in this._context.NodeMangs
                        ...
                        break;
                    case "4": //流程起始者
                        var rNodeInit = (from c in this._context.NodeInits
                                         where c.FlowNode_id == rFlowNode.id
                                         select c).FirstOrDefault();
                        ...
                        break;
                    case "5": //會簽起始者
                        var rNodeMultiInit = (from c in this._context.NodeMultiInits
                                              where c.FlowNode_id == rFlowNode.id
                                              select c).FirstOrDefault();
                    ...
                    case "6": //自定簽核者
                        ...
                        break;
                    case "7": //動態簽核者
                        var rNodeDynamic = (from c in this._context.NodeDynamics
                                            where c.FlowNode_id == rFlowNode.id
                                            select c).FirstOrDefault();
                        ...
                        break;
                    case "8": //流程代理者
                        var rNodeAgentInit = (from c in this._context.NodeAgentInits
                                              where c.FlowNode_id == rFlowNode.id
                                              select c).FirstOrDefault();
        ...
        public string GetFlowParmUrl(int iApParmID, bool bOnlyUrl = false)
        {
            ...
            if (iApParmID != 0)
            {
                var rSysVar = (from c in this._context.SysVars
                               select c).FirstOrDefault();
                ...
                if (rSysVar != null)
                {
                    ...
                    if (rProcessApParm != null)
                    {
                        ...
                        if (rProcessFlow != null)
                        {
                            ...
                            if (rFlowNode != null)
                            {
                                var rNodeStart = (from c in this._context.NodeStarts
                                                  where c.FlowNode_id == rFlowNode.id
                                                  select c).FirstOrDefault();
                                ...
                                if (rNodeStart != null)
                                {
                                    ...
                                    {
                                        var rFlowNodeCheck = (from c in this._context.FlowNodes
        ...
        public List<FlowSearchCompleteRow> GetFlowSearchComplete(string sNobr, DateTime dAppB, DateTime dAppE)
        {
            var Vdb = (from pf in this._context.ProcessFlows
                       join pav in this._context.ProcessApViews on pf.id equals pav.ProcessFlow_id.Value
        ...
        {
            var ProcessSql = from pf in this._context.ProcessFlows
                             ...
                             join pa in this._context.ProcessApViews on pf.id equals pa.ProcessFlow_id
                             join ft in this._context.FlowTrees on pf.FlowTree_id equals ft.id
                             join fn in this._context.FlowNodes on pn.FlowNode_id equals fn.id
                             join pfs in this._context.ProcessFlowShares on pf.id equals pfs.ProcessFlow_id
                             join e1 in this._context.Emps on pc.Emp_idAgent equals e1.id into pc1
                             from pc1Row in pc1.DefaultIfEmpty()
                             join e2 in this._context.Emps on pc.Emp_idDefault equals e2.id into pc2
        ...
        {
            var ProcessSql = from pc in this._context.ProcessChecks
                             join pn in this._context.ProcessNodes on pc.ProcessNode_auto equals pn.auto
                             join pf in this._context.ProcessFlows on pn.ProcessFlow_id equals pf.id
                             join fn in this._context.FlowNodes on pn.FlowNode_id equals fn.id
                             join ft in this._context.FlowTrees on pf.FlowTree_id equals ft.id
                             join e1 in this._context.Emps on pc.Emp_idDefault equals e1.id into pc1
                             from pc1Row in pc1.DefaultIfEmpty()
                             join e2 in this._context.Emps on pc.Emp_idAgent equals e2.id into pc2
                             from pc2Row in pc2.DefaultIfEmpty()
            ...
            //填入表單資訊
            var rsFormApp = (from c in this._context.wfFormApps
            ...
            var rsProcessApParm = (from c in this._context.ProcessApParms
        ...
        {
            var rsFlowTree = (from t in this._context.FlowTrees
            ...
            var rsFlowTreePower = (from t in this._context.FlowTreePowers
                                   select t).ToList();

            var rsFlowTreePowerRoleOnly = (from t in this._context.FlowTreePowerRoleOnlies
                                           select t).ToList();

            var rsFlowNode = (from n in this._context.FlowNodes
                              join ns in this._context.NodeStarts on n.id equals ns.FlowNode_id into nns
                              from nnsRow in nns.DefaultIfEmpty()
                              join nf in this._context.NodeForms on n.id equals nf.FlowNode_id into nnf
            ...
            var rsRole = (from r in this._context.Roles
                          join d in this._context.Depts
                          on r.Dept_id equals d.id
                          join p in this._context.Pos
                          on r.Pos_id equals p.id
                          join e in this._context.Emps
        ...
        {
            var Vdb = (from fn in this._context.FlowNodes
                       join ns in this._context.NodeStarts on fn.id equals ns.FlowNode_id
                       where fn.nodeType == "1"
                       && (lsFlowTreeId == null
        ...
        public List<FlowViewRow> GetFlowView(bool bManage = false, string sNobr = "", DateTime? dDateSignB = null, DateTime? dDateSignE = null, DateTime? dDateAppB = null, DateTime? dDateAppE = null, string sState = "1", string sFormCode = "0", int iProcessID = 0, string sApp = "1")
        {
            ...
            var arrForm = (from c in this._context.wfForms
                           where c.iSort > 0
                           orderby c.iSort
            ...
            {
                //審核者
                var rsProcessNode = (from pn in this._context.ProcessNodes
                                     join pc in this._context.ProcessChecks on pn.auto equals pc.ProcessNode_auto
                ...
                //被申請人
                var rsFormAppInfo = (from s in this._context.wfFormAppInfos
                ...
                //申請人
                Vdb = (from m in this._context.wfFormApps
                       join pf in this._context.ProcessFlows on m.idProcess equals pf.id
                       join f in this._context.wfForms on pf.FlowTree_id equals f.sFlowTree
        ...
        public string GetFlowViewUrl(int idProcess, bool bOnlyUrl = false)
        {
            ...
            var rNodeStart = (from pf in this._context.ProcessFlows
                              join fn in this._context.FlowNodes on pf.FlowTree_id equals fn.FlowTree_id
                              join ns in this._context.NodeStarts on fn.id equals ns.FlowNode_id
        ...
        public List<ProcessDataRow> GetProcessData(List<int> lsProcessID)
        {
            ...
            var rsProcessMultiFlow = (from pn in this._context.ProcessMultiFlows
                                      where (from pf in rsProcessFlowSql where pn.ProcessFlow_id == pf.id select 1).Any()
                                      select pn).ToList();
            ...
            foreach (var rProcessFlow in rsProcessFlow)
            {
                ...
                var rEmp = (from c in this._context.Emps
                ...
                if (rFormApp != null)
                {
                    ...
                    rProcessDataRow.DateTimeD = rFormApp.DateTimeD.GetValueOrDefault(new DateTime(1900, 1, 1));
                    rProcessDataRow.FlowAppName = "";   //表單自訂名稱 
                }
                ...
                if (rsProcessNodeWhere.Count > 0)
                {
                    ...
                    foreach (var rProcessNode in rsProcessNodeWhere)
                    {
                        ...
                        if (rProcessCheck != null)
                        {
                            ...
                            rProcessDataRow.Role_idDefault = rProcessCheck.Role_idDefault;
                            ...
                            rEmp = (from c in this._context.Emps
