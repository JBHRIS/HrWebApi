...
namespace Repo
{
    ...
    public class FileStructureRole_Repo
    {
        ...
        public FileStructureRole_Repo(dcTrainingDataContext d)
        {
            dc = d;            
        }
        ...
        public List<FileStructureRole> GetByRoleKey(int Avalue)
        {
            ...
            {
                return (from c in ldc.FileStructureRole
                        where c.RoleKey.Equals(Avalue)
                        select c).ToList();
            }
        }

        public FileStructureRole GetByFKeyRoleKey(string Afkey,int AroleKey)
        {
            using (dcTrainingDataContext ldc = new dcTrainingDataContext())
            {
                return (from c in ldc.FileStructureRole
                        where c.RoleKey.Equals(AroleKey) && c.FileStructureKey.Equals(Afkey)
